 ...
 * contrary to what is driver supposed to करो. However, values are not
 * really used, so this might be acceptable. It's possible that bitstream
 * parsing functions set some पूर्णांकernal VPU state, which is later necessary
 * क्रम proper decoding. Biggest suspect is "VP8 probs update" trigger.
 */
...
#घोषणा CEDRUS_ENTROPY_PROBS_SIZE 0x2400
#घोषणा VP8_PROB_HALF 128
#घोषणा QUANT_DELTA_COUNT 5

/*
 * This table comes from the concatenation of k_coeff_entropy_update_probs,
 * kf_ymode_prob, शेष_mv_context, etc. It is provided in this क्रमm in
 * order to aव्योम computing it every समय the driver is initialised, and is
 * suitable क्रम direct consumption by the hardware.
 */
अटल स्थिर u8 prob_table_init[] = अणु
 ...
 */
अटल स्थिर u8 k_mv_entropy_update_probs[2][V4L2_VP8_MV_PROB_CNT] = अणु
	अणु 237, 246, 253, 253, 254, 254, 254, 254, 254,
	  254, 254, 254, 254, 254, 250, 250, 252, 254, 254 पूर्ण,
	अणु 231, 243, 245, 253, 254, 254, 254, 254, 254,
	  254, 254, 254, 254, 254, 251, 251, 254, 254, 254 पूर्ण
पूर्ण;

अटल uपूर्णांक8_t पढ़ो_bits(काष्ठा cedrus_dev *dev, अचिन्हित पूर्णांक bits_count,
			 अचिन्हित पूर्णांक probability)
अणु
	...
	cedrus_रुको_क्रम(dev, VE_H264_STATUS, VE_H264_STATUS_VLD_BUSY);

	वापस cedrus_पढ़ो(dev, VE_H264_BASIC_BITS);
पूर्ण

अटल व्योम get_delta_q(काष्ठा cedrus_dev *dev)
अणु
	अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF)) अणु
		पढ़ो_bits(dev, 4, VP8_PROB_HALF);
		पढ़ो_bits(dev, 1, VP8_PROB_HALF);
	पूर्ण
पूर्ण

अटल व्योम process_segmentation_info(काष्ठा cedrus_dev *dev)
अणु
	पूर्णांक update, i;

	update = पढ़ो_bits(dev, 1, VP8_PROB_HALF);
	...
	अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF)) अणु
		...
		क्रम (i = 0; i < 4; i++)
			अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF)) अणु
				पढ़ो_bits(dev, 7, VP8_PROB_HALF);
				पढ़ो_bits(dev, 1, VP8_PROB_HALF);
			पूर्ण

		क्रम (i = 0; i < 4; i++)
			अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF)) अणु
				पढ़ो_bits(dev, 6, VP8_PROB_HALF);
				पढ़ो_bits(dev, 1, VP8_PROB_HALF);
			पूर्ण
	पूर्ण
	...
	अगर (update)
		क्रम (i = 0; i < 3; i++)
			अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF))
				पढ़ो_bits(dev, 8, VP8_PROB_HALF);
पूर्ण

अटल व्योम process_ref_lf_delta_info(काष्ठा cedrus_dev *dev)
अणु
	अगर (पढ़ो_bits(dev, 1, VP8_PROB_HALF)) अणु
		पूर्णांक i;
...
अटल व्योम process_ref_frame_info(काष्ठा cedrus_dev *dev)
अणु
	u8 refresh_golden_frame = पढ़ो_bits(dev, 1, VP8_PROB_HALF);
	u8 refresh_alt_ref_frame = पढ़ो_bits(dev, 1, VP8_PROB_HALF);

	अगर (!refresh_golden_frame)
		पढ़ो_bits(dev, 2, VP8_PROB_HALF);

	अगर (!refresh_alt_ref_frame)
		पढ़ो_bits(dev, 2, VP8_PROB_HALF);
...
अटल व्योम cedrus_irq_clear(काष्ठा cedrus_dev *dev)
अणु
	cedrus_ग_लिखो(dev, VE_H264_STATUS,
		     VE_H264_STATUS_INT_MASK);
पूर्ण

अटल व्योम cedrus_पढ़ो_header(काष्ठा cedrus_dev *dev,
			       स्थिर काष्ठा v4l2_ctrl_vp8_frame *slice)
अणु
	पूर्णांक i, j;

	अगर (V4L2_VP8_FRAME_IS_KEY_FRAME(slice)) अणु
		पढ़ो_bits(dev, 1, VP8_PROB_HALF);
		पढ़ो_bits(dev, 1, VP8_PROB_HALF);
	पूर्ण
	...
	क्रम (i = 0; i < QUANT_DELTA_COUNT; i++)
		get_delta_q(dev);

	अगर (!V4L2_VP8_FRAME_IS_KEY_FRAME(slice))
	...
	cedrus_ग_लिखो(dev, VE_H264_TRIGGER_TYPE, VE_H264_TRIGGER_TYPE_VP8_UPDATE_COEF);
	cedrus_रुको_क्रम(dev, VE_H264_STATUS, VE_H264_STATUS_VP8_UPPROB_BUSY);
	...
	अगर (!V4L2_VP8_FRAME_IS_KEY_FRAME(slice)) अणु
		...
		क्रम (i = 0; i < 2; i++)
			क्रम (j = 0; j < V4L2_VP8_MV_PROB_CNT; j++)
				अगर (पढ़ो_bits(dev, 1, k_mv_entropy_update_probs[i][j]))
					पढ़ो_bits(dev, 7, VP8_PROB_HALF);
	पूर्ण
पूर्ण

अटल व्योम cedrus_vp8_update_probs(स्थिर काष्ठा v4l2_ctrl_vp8_frame *slice,
				    u8 *prob_table)
अणु
	पूर्णांक i, j, k;

	स_नकल(&prob_table[0x1008], slice->entropy.y_mode_probs,
	       माप(slice->entropy.y_mode_probs));
	स_नकल(&prob_table[0x1010], slice->entropy.uv_mode_probs,
	       माप(slice->entropy.uv_mode_probs));

	स_नकल(&prob_table[0x1018], slice->segment.segment_probs,
	       माप(slice->segment.segment_probs));

	prob_table[0x101c] = slice->prob_skip_false;
	prob_table[0x101d] = slice->prob_पूर्णांकra;
	prob_table[0x101e] = slice->prob_last;
	prob_table[0x101f] = slice->prob_gf;

	स_नकल(&prob_table[0x1020], slice->entropy.mv_probs[0],
	       V4L2_VP8_MV_PROB_CNT);
	स_नकल(&prob_table[0x1040], slice->entropy.mv_probs[1],
	       V4L2_VP8_MV_PROB_CNT);

	क्रम (i = 0; i < 4; ++i)
		क्रम (j = 0; j < 8; ++j)
			क्रम (k = 0; k < 3; ++k)
				स_नकल(&prob_table[i * 512 + j * 64 + k * 16],
				       slice->entropy.coeff_probs[i][j][k], 11);
पूर्ण

अटल क्रमागत cedrus_irq_status
cedrus_vp8_irq_status(काष्ठा cedrus_ctx *ctx)
अणु
...
अटल व्योम cedrus_vp8_irq_clear(काष्ठा cedrus_ctx *ctx)
अणु
	cedrus_irq_clear(ctx->dev);
पूर्ण

अटल व्योम cedrus_vp8_irq_disable(काष्ठा cedrus_ctx *ctx)
अणु
	काष्ठा cedrus_dev *dev = ctx->dev;
...
अटल व्योम cedrus_vp8_setup(काष्ठा cedrus_ctx *ctx,
			     काष्ठा cedrus_run *run)
अणु
	स्थिर काष्ठा v4l2_ctrl_vp8_frame *slice = run->vp8.frame_params;
	काष्ठा vb2_queue *cap_q = &ctx->fh.m2m_ctx->cap_q_ctx.q;
	...
	cedrus_ग_लिखो(dev, VE_H264_CTRL, VE_H264_CTRL_VP8);

	cedrus_vp8_update_probs(slice, ctx->codec.vp8.entropy_probs_buf);

	reg = slice->first_part_size * 8;
	cedrus_ग_लिखो(dev, VE_VP8_FIRST_DATA_PART_LEN, reg);

	header_size = V4L2_VP8_FRAME_IS_KEY_FRAME(slice) ? 10 : 3;

	reg = slice->first_part_size + header_size;
	cedrus_ग_लिखो(dev, VE_VP8_PART_SIZE_OFFSET, reg);

	reg = vb2_plane_size(src_buf, 0) * 8;
	cedrus_ग_लिखो(dev, VE_H264_VLD_LEN, reg);

	/*
	 * FIXME: There is a problem अगर frame header is skipped (adding
	 * first_part_header_bits to offset). It seems that functions
	 * क्रम parsing bitstreams change पूर्णांकernal state of VPU in some
	 * way that can't be otherwise set. Maybe this can be bypassed
	 * by somehow fixing probability table buffer?
	 */
	reg = header_size * 8;
	cedrus_ग_लिखो(dev, VE_H264_VLD_OFFSET, reg);

	src_buf_addr = vb2_dma_contig_plane_dma_addr(src_buf, 0);
	cedrus_ग_लिखो(dev, VE_H264_VLD_END,
	...
	cedrus_ग_लिखो(dev, VE_VP8_ENTROPY_PROBS_ADDR,
		     ctx->codec.vp8.entropy_probs_buf_dma);

	reg = 0;
	चयन (slice->version) अणु
	हाल 1:
	...
	हाल 3:
		...
		अवरोध;
	पूर्ण
	अगर (slice->segment.flags & V4L2_VP8_SEGMENT_FLAG_UPDATE_MAP)
		reg |= VE_VP8_PPS_UPDATE_MB_SEGMENTATION_MAP;
	अगर (!(slice->segment.flags & V4L2_VP8_SEGMENT_FLAG_DELTA_VALUE_MODE))
		reg |= VE_VP8_PPS_MB_SEGMENT_ABS_DELTA;
	अगर (slice->segment.flags & V4L2_VP8_SEGMENT_FLAG_ENABLED)
		reg |= VE_VP8_PPS_SEGMENTATION_ENABLE;
	अगर (ctx->codec.vp8.last_filter_type)
		reg |= VE_VP8_PPS_LAST_LOOP_FILTER_SIMPLE;
	reg |= VE_VP8_PPS_SHARPNESS_LEVEL(slice->lf.sharpness_level);
	अगर (slice->lf.flags & V4L2_VP8_LF_FILTER_TYPE_SIMPLE)
		reg |= VE_VP8_PPS_LOOP_FILTER_SIMPLE;
	reg |= VE_VP8_PPS_LOOP_FILTER_LEVEL(slice->lf.level);
	अगर (slice->lf.flags & V4L2_VP8_LF_ADJ_ENABLE)
		reg |= VE_VP8_PPS_MODE_REF_LF_DELTA_ENABLE;
	अगर (slice->lf.flags & V4L2_VP8_LF_DELTA_UPDATE)
		reg |= VE_VP8_PPS_MODE_REF_LF_DELTA_UPDATE;
	reg |= VE_VP8_PPS_TOKEN_PARTITION(ilog2(slice->num_dct_parts));
	अगर (slice->flags & V4L2_VP8_FRAME_FLAG_MB_NO_SKIP_COEFF)
		reg |= VE_VP8_PPS_MB_NO_COEFF_SKIP;
	reg |= VE_VP8_PPS_RELOAD_ENTROPY_PROBS;
	अगर (slice->flags & V4L2_VP8_FRAME_FLAG_SIGN_BIAS_GOLDEN)
		reg |= VE_VP8_PPS_GOLDEN_SIGN_BIAS;
	अगर (slice->flags & V4L2_VP8_FRAME_FLAG_SIGN_BIAS_ALT)
		reg |= VE_VP8_PPS_ALTREF_SIGN_BIAS;
	अगर (ctx->codec.vp8.last_frame_p_type)
		reg |= VE_VP8_PPS_LAST_PIC_TYPE_P_FRAME;
	reg |= VE_VP8_PPS_LAST_SHARPNESS_LEVEL(ctx->codec.vp8.last_sharpness_level);
	अगर (!(slice->flags & V4L2_VP8_FRAME_FLAG_KEY_FRAME))
		reg |= VE_VP8_PPS_PIC_TYPE_P_FRAME;
	cedrus_ग_लिखो(dev, VE_VP8_PPS, reg);

	cedrus_पढ़ो_header(dev, slice);

	/* reset रेजिस्टरs changed by HW */
	cedrus_ग_लिखो(dev, VE_H264_CUR_MB_NUM, 0);
	cedrus_ग_लिखो(dev, VE_H264_MB_ADDR, 0);
	cedrus_ग_लिखो(dev, VE_H264_ERROR_CASE, 0);
	...
	cedrus_ग_लिखो(dev, VE_VP8_SEGMENT_FEAT_MB_LV0, reg);
	...
	cedrus_ग_लिखो(dev, VE_VP8_SEGMENT_FEAT_MB_LV1, reg);
	...
	cedrus_ग_लिखो(dev, VE_VP8_REF_LF_DELTA, reg);
	...
	cedrus_ग_लिखो(dev, VE_VP8_MODE_LF_DELTA, reg);

	luma_addr = cedrus_dst_buf_addr(ctx, run->dst->vb2_buf.index, 0);
	chroma_addr = cedrus_dst_buf_addr(ctx, run->dst->vb2_buf.index, 1);
	cedrus_ग_लिखो(dev, VE_VP8_REC_LUMA, luma_addr);
	cedrus_ग_लिखो(dev, VE_VP8_REC_CHROMA, chroma_addr);

	qindex = vb2_find_बारtamp(cap_q, slice->last_frame_ts, 0);
	अगर (qindex >= 0) अणु
		luma_addr = cedrus_dst_buf_addr(ctx, qindex, 0);
		chroma_addr = cedrus_dst_buf_addr(ctx, qindex, 1);
		cedrus_ग_लिखो(dev, VE_VP8_FWD_LUMA, luma_addr);
		cedrus_ग_लिखो(dev, VE_VP8_FWD_CHROMA, chroma_addr);
	पूर्ण अन्यथा अणु
		cedrus_ग_लिखो(dev, VE_VP8_FWD_LUMA, 0);
		cedrus_ग_लिखो(dev, VE_VP8_FWD_CHROMA, 0);
	पूर्ण

	qindex = vb2_find_बारtamp(cap_q, slice->golden_frame_ts, 0);
	अगर (qindex >= 0) अणु
		luma_addr = cedrus_dst_buf_addr(ctx, qindex, 0);
		chroma_addr = cedrus_dst_buf_addr(ctx, qindex, 1);
		cedrus_ग_लिखो(dev, VE_VP8_BWD_LUMA, luma_addr);
		cedrus_ग_लिखो(dev, VE_VP8_BWD_CHROMA, chroma_addr);
	पूर्ण अन्यथा अणु
		cedrus_ग_लिखो(dev, VE_VP8_BWD_LUMA, 0);
		cedrus_ग_लिखो(dev, VE_VP8_BWD_CHROMA, 0);
	पूर्ण

	qindex = vb2_find_बारtamp(cap_q, slice->alt_frame_ts, 0);
	अगर (qindex >= 0) अणु
		luma_addr = cedrus_dst_buf_addr(ctx, qindex, 0);
		chroma_addr = cedrus_dst_buf_addr(ctx, qindex, 1);
		cedrus_ग_लिखो(dev, VE_VP8_ALT_LUMA, luma_addr);
		cedrus_ग_लिखो(dev, VE_VP8_ALT_CHROMA, chroma_addr);
	पूर्ण अन्यथा अणु
		cedrus_ग_लिखो(dev, VE_VP8_ALT_LUMA, 0);
		cedrus_ग_लिखो(dev, VE_VP8_ALT_CHROMA, 0);
	पूर्ण
...
अटल पूर्णांक cedrus_vp8_start(काष्ठा cedrus_ctx *ctx)
अणु
	काष्ठा cedrus_dev *dev = ctx->dev;
	...
	ctx->codec.vp8.entropy_probs_buf =
		dma_alloc_coherent(dev->dev, CEDRUS_ENTROPY_PROBS_SIZE,
				   &ctx->codec.vp8.entropy_probs_buf_dma,
				   GFP_KERNEL);
	अगर (!ctx->codec.vp8.entropy_probs_buf)
		वापस -ENOMEM;

	/*
	 * This offset has been discovered by reverse engineering, we करोnै t know
	 * what it actually means.
	 */
	स_नकल(&ctx->codec.vp8.entropy_probs_buf[2048],
	       prob_table_init, माप(prob_table_init));

	वापस 0;
पूर्ण

अटल व्योम cedrus_vp8_stop(काष्ठा cedrus_ctx *ctx)
अणु
	काष्ठा cedrus_dev *dev = ctx->dev;

	cedrus_engine_disable(dev);

	dma_मुक्त_coherent(dev->dev, CEDRUS_ENTROPY_PROBS_SIZE,
			  ctx->codec.vp8.entropy_probs_buf,
			  ctx->codec.vp8.entropy_probs_buf_dma);
पूर्ण

अटल व्योम cedrus_vp8_trigger(काष्ठा cedrus_ctx *ctx)
अणु
	काष्ठा cedrus_dev *dev = ctx->dev;
...
काष्ठा cedrus_dec_ops cedrus_dec_ops_vp8 = अणु
