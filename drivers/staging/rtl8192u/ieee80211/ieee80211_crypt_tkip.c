...
/*
 * Host AP crypt: host-based TKIP encryption implementation क्रम Host AP driver
 *
...
काष्ठा ieee80211_tkip_data अणु
...
अटल व्योम *ieee80211_tkip_init(पूर्णांक key_idx)
अणु
	काष्ठा ieee80211_tkip_data *priv;
	...
	अगर (IS_ERR(priv->tx_tfm_michael)) अणु
		prपूर्णांकk(KERN_DEBUG "ieee80211_crypt_tkip: could not allocate "
...
अटल व्योम ieee80211_tkip_deinit(व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *_priv = priv;
...
अटल व्योम tkip_mixing_phase1(u16 *TTAK, स्थिर u8 *TK, स्थिर u8 *TA, u32 IV32)
अणु
	पूर्णांक i, j;
...
अटल पूर्णांक ieee80211_tkip_encrypt(काष्ठा sk_buff *skb, पूर्णांक hdr_len, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkey = priv;
	पूर्णांक len;
	u8 *pos;
	...
	अगर (skb_headroom(skb) < 8 || skb_tailroom(skb) < 4 ||
	    skb->len < hdr_len)
		वापस -1;
...
अटल पूर्णांक ieee80211_tkip_decrypt(काष्ठा sk_buff *skb, पूर्णांक hdr_len, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkey = priv;
	u8 keyidx, *pos;
	u32 iv32;
	...
	अगर (skb->len < hdr_len + 8 + 4)
		वापस -1;

	hdr = (काष्ठा rtl_80211_hdr_4addr *)skb->data;
	pos = skb->data + hdr_len;
	keyidx = pos[3];
	अगर (!(keyidx & BIT(5))) अणु
	...
	अगर (!tcb_desc->bHwSec) अणु
		अगर (iv32 < tkey->rx_iv32 ||
		(iv32 == tkey->rx_iv32 && iv16 <= tkey->rx_iv16)) अणु
			अगर (net_ratelimit()) अणु
		...
		अगर (स_भेद(icv, pos + plen, 4) != 0) अणु
			अगर (iv32 != tkey->rx_iv32) अणु
				/*
				 * Previously cached Phase1 result was alपढ़ोy
				 * lost, so it needs to be recalculated क्रम the
				 * next packet.
				 */
	...
	/* Remove IV and ICV */
	स_हटाओ(skb->data + 8, skb->data, hdr_len);
	skb_pull(skb, 8);
...
अणु
	काष्ठा rtl_80211_hdr_4addr *hdr11;

	hdr11 = (काष्ठा rtl_80211_hdr_4addr *)skb->data;
	चयन (le16_to_cpu(hdr11->frame_ctl) &
...
अटल पूर्णांक ieee80211_michael_mic_add(काष्ठा sk_buff *skb, पूर्णांक hdr_len, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkey = priv;
	u8 *pos;
	काष्ठा rtl_80211_hdr_4addr *hdr;
	...
	अगर (IEEE80211_QOS_HAS_SEQ(le16_to_cpu(hdr->frame_ctl)))
...
अटल व्योम ieee80211_michael_mic_failure(काष्ठा net_device *dev,
				       काष्ठा rtl_80211_hdr_4addr *hdr,
				       पूर्णांक keyidx)
अणु
...
अटल पूर्णांक ieee80211_michael_mic_verअगरy(काष्ठा sk_buff *skb, पूर्णांक keyidx,
...
अटल पूर्णांक ieee80211_tkip_set_key(व्योम *key, पूर्णांक len, u8 *seq, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkey = priv;
	पूर्णांक keyidx;
...
अटल पूर्णांक ieee80211_tkip_get_key(व्योम *key, पूर्णांक len, u8 *seq, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkey = priv;
...
अटल अक्षर *ieee80211_tkip_prपूर्णांक_stats(अक्षर *p, व्योम *priv)
अणु
	काष्ठा ieee80211_tkip_data *tkip = priv;

	p += प्र_लिखो(p, "key[%d] alg=TKIP key_set=%d "
...
अटल काष्ठा ieee80211_crypto_ops ieee80211_crypt_tkip = अणु
	.name			= "TKIP",
	...
	.decrypt_msdu		= ieee80211_michael_mic_verअगरy,
	.set_key		= ieee80211_tkip_set_key,
	.get_key		= ieee80211_tkip_get_key,
	.prपूर्णांक_stats		= ieee80211_tkip_prपूर्णांक_stats,
	.extra_prefix_len	= 4 + 4, /* IV + ExtIV */
	.extra_postfix_len	= 8 + 4, /* MIC + ICV */
	.owner			= THIS_MODULE,
पूर्ण;

पूर्णांक __init ieee80211_crypto_tkip_init(व्योम)
अणु
	वापस ieee80211_रेजिस्टर_crypto_ops(&ieee80211_crypt_tkip);
पूर्ण

व्योम __निकास ieee80211_crypto_tkip_निकास(व्योम)
अणु
	ieee80211_unरेजिस्टर_crypto_ops(&ieee80211_crypt_tkip);
पूर्ण
