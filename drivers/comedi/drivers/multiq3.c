<शैली गुरु>
// SPDX-License-Identअगरier: GPL-2.0+
/*
 * multiq3.c
 * Hardware driver क्रम Quanser Consulting MultiQ-3 board
 *
 * COMEDI - Linux Control and Measurement Device Interface
 * Copyright (C) 1999 Anders Blomdell <anders.blomdell@control.lth.se>
 */

/*
 * Driver: multiq3
 * Description: Quanser Consulting MultiQ-3
 * Devices: [Quanser Consulting] MultiQ-3 (multiq3)
 * Author: Anders Blomdell <anders.blomdell@control.lth.se>
 * Status: works
 *
 * Configuration Options:
 *  [0] - I/O port base address
 *  [1] - IRQ (not used)
 *  [2] - Number of optional encoder chips installed on board
 *	  0 = none
 *	  1 = 2 inमाला_दो (Model -2E)
 *	  2 = 4 inमाला_दो (Model -4E)
 *	  3 = 6 inमाला_दो (Model -6E)
 *	  4 = 8 inमाला_दो (Model -8E)
 */

#समावेश <linux/module.h>

#समावेश "../comedidev.h"

/*
 * Register map
 */
#घोषणा MULTIQ3_DI_REG			0x00
#घोषणा MULTIQ3_DO_REG			0x00
#घोषणा MULTIQ3_AO_REG			0x02
#घोषणा MULTIQ3_AI_REG			0x04
#घोषणा MULTIQ3_AI_CONV_REG		0x04
#घोषणा MULTIQ3_STATUS_REG		0x06
#घोषणा MULTIQ3_STATUS_EOC		BIT(3)
#घोषणा MULTIQ3_STATUS_EOC_I		BIT(4)
#घोषणा MULTIQ3_CTRL_REG		0x06
#घोषणा MULTIQ3_CTRL_AO_CHAN(x)		(((x) & 0x7) << 0)
#घोषणा MULTIQ3_CTRL_RC(x)		(((x) & 0x3) << 0)
#घोषणा MULTIQ3_CTRL_AI_CHAN(x)		(((x) & 0x7) << 3)
#घोषणा MULTIQ3_CTRL_E_CHAN(x)		(((x) & 0x7) << 3)
#घोषणा MULTIQ3_CTRL_EN			BIT(6)
#घोषणा MULTIQ3_CTRL_AZ			BIT(7)
#घोषणा MULTIQ3_CTRL_CAL		BIT(8)
#घोषणा MULTIQ3_CTRL_SH			BIT(9)
#घोषणा MULTIQ3_CTRL_CLK		BIT(10)
#घोषणा MULTIQ3_CTRL_LD			(3 << 11)
#घोषणा MULTIQ3_CLK_REG			0x08
#घोषणा MULTIQ3_ENC_DATA_REG		0x0c
#घोषणा MULTIQ3_ENC_CTRL_REG		0x0e

/*
 * Encoder chip commands (from the programming manual)
 */
#घोषणा MULTIQ3_CLOCK_DATA		0x00	/* FCK frequency भागider */
#घोषणा MULTIQ3_CLOCK_SETUP		0x18	/* xfer PR0 to PSC */
#घोषणा MULTIQ3_INPUT_SETUP		0x41	/* enable inमाला_दो A and B */
#घोषणा MULTIQ3_QUAD_X4			0x38	/* quadrature */
#घोषणा MULTIQ3_BP_RESET		0x01	/* reset byte poपूर्णांकer */
#घोषणा MULTIQ3_CNTR_RESET		0x02	/* reset counter */
#घोषणा MULTIQ3_TRSFRPR_CTR		0x08	/* xfre preset reg to counter */
#घोषणा MULTIQ3_TRSFRCNTR_OL		0x10	/* xfer CNTR to OL (x and y) */
#घोषणा MULTIQ3_EFLAG_RESET		0x06	/* reset E bit of flag reg */

अटल व्योम multiq3_set_ctrl(काष्ठा comedi_device *dev, अचिन्हित पूर्णांक bits)
अणु
	/*
	 * According to the programming manual, the SH and CLK bits should
	 * be kept high at all बार.
	 */
	outw(MULTIQ3_CTRL_SH | MULTIQ3_CTRL_CLK | bits,
	     dev->iobase + MULTIQ3_CTRL_REG);
पूर्ण

अटल पूर्णांक multiq3_ai_status(काष्ठा comedi_device *dev,
			     काष्ठा comedi_subdevice *s,
			     काष्ठा comedi_insn *insn,
			     अचिन्हित दीर्घ context)
अणु
	अचिन्हित पूर्णांक status;

	status = inw(dev->iobase + MULTIQ3_STATUS_REG);
	अगर (status & context)
		वापस 0;
	वापस -EBUSY;
पूर्ण

अटल पूर्णांक multiq3_ai_insn_पढ़ो(काष्ठा comedi_device *dev,
				काष्ठा comedi_subdevice *s,
				काष्ठा comedi_insn *insn,
				अचिन्हित पूर्णांक *data)
अणु
	अचिन्हित पूर्णांक chan = CR_CHAN(insn->chanspec);
	अचिन्हित पूर्णांक val;
	पूर्णांक ret;
	पूर्णांक i;

	multiq3_set_ctrl(dev, MULTIQ3_CTRL_EN | MULTIQ3_CTRL_AI_CHAN(chan));

	ret = comedi_समयout(dev, s, insn, multiq3_ai_status,
			     MULTIQ3_STATUS_EOC);
	अगर (ret)
		वापस ret;

	क्रम (i = 0; i < insn->n; i++) अणु
		outw(0, dev->iobase + MULTIQ3_AI_CONV_REG);

		ret = comedi_समयout(dev, s, insn, multiq3_ai_status,
				     MULTIQ3_STATUS_EOC_I);
		अगर (ret)
			वापस ret;

		/* get a 16-bit sample; mask it to the subdevice resolution */
		val = inb(dev->iobase + MULTIQ3_AI_REG) << 8;
		val |= inb(dev->iobase + MULTIQ3_AI_REG);
		val &= s->maxdata;

		/* munge the 2's complement value to offset binary */
		data[i] = comedi_offset_munge(s, val);
	पूर्ण

	वापस insn->n;
पूर्ण

अटल पूर्णांक multiq3_ao_insn_ग_लिखो(काष्ठा comedi_device *dev,
				 काष्ठा comedi_subdevice *s,
				 काष्ठा comedi_insn *insn,
				 अचिन्हित पूर्णांक *data)
अणु
	अचिन्हित पूर्णांक chan = CR_CHAN(insn->chanspec);
	अचिन्हित पूर्णांक val = s->पढ़ोback[chan];
	पूर्णांक i;

	क्रम (i = 0; i < insn->n; i++) अणु
		val = data[i];
		multiq3_set_ctrl(dev, MULTIQ3_CTRL_LD |
				      MULTIQ3_CTRL_AO_CHAN(chan));
		outw(val, dev->iobase + MULTIQ3_AO_REG);
		multiq3_set_ctrl(dev, 0);
	पूर्ण
	s->पढ़ोback[chan] = val;

	वापस insn->n;
पूर्ण

अटल पूर्णांक multiq3_di_insn_bits(काष्ठा comedi_device *dev,
				काष्ठा comedi_subdevice *s,
				काष्ठा comedi_insn *insn, अचिन्हित पूर्णांक *data)
अणु
	data[1] = inw(dev->iobase + MULTIQ3_DI_REG);

	वापस insn->n;
पूर्ण

अटल पूर्णांक multiq3_करो_insn_bits(काष्ठा comedi_device *dev,
				काष्ठा comedi_subdevice *s,
				काष्ठा comedi_insn *insn,
				अचिन्हित पूर्णांक *data)
अणु
	अगर (comedi_dio_update_state(s, data))
		outw(s->state, dev->iobase + MULTIQ3_DO_REG);

	data[1] = s->state;

	वापस insn->n;
पूर्ण

अटल पूर्णांक multiq3_encoder_insn_पढ़ो(काष्ठा comedi_device *dev,
				     काष्ठा comedi_subdevice *s,
				     काष्ठा comedi_insn *insn,
				     अचिन्हित पूर्णांक *data)
अणु
	अचिन्हित पूर्णांक chan = CR_CHAN(insn->chanspec);
	अचिन्हित पूर्णांक val;
	पूर्णांक i;

	क्रम (i = 0; i < insn->n; i++) अणु
		/* select encoder channel */
		multiq3_set_ctrl(dev, MULTIQ3_CTRL_EN |
				      MULTIQ3_CTRL_E_CHAN(chan));

		/* reset the byte poपूर्णांकer */
		outb(MULTIQ3_BP_RESET, dev->iobase + MULTIQ3_ENC_CTRL_REG);

		/* latch the data */
		outb(MULTIQ3_TRSFRCNTR_OL, dev->iobase + MULTIQ3_ENC_CTRL_REG);

		/* पढ़ो the 24-bit encoder data (lsb/mid/msb) */
		val = inb(dev->iobase + MULTIQ3_ENC_DATA_REG);
		val |= (inb(dev->iobase + MULTIQ3_ENC_DATA_REG) << 8);
		val |= (inb(dev->iobase + MULTIQ3_ENC_DATA_REG) << 16);

		/*
		 * Munge the data so that the reset value is in the middle
		 * of the maxdata range, i.e.:
		 *
		 * real value	comedi value
		 * 0xffffff	0x7fffff	1 negative count
		 * 0x000000	0x800000	reset value
		 * 0x000001	0x800001	1 positive count
		 *
		 * It's possible क्रम the 24-bit counter to overflow but it
		 * would normally take _quite_ a few turns. A 2000 line
		 * encoder in quadrature results in 8000 counts/rev. So about
		 * 1048 turns in either direction can be measured without
		 * an overflow.
		 */
		data[i] = (val + ((s->maxdata + 1) >> 1)) & s->maxdata;
	पूर्ण

	वापस insn->n;
पूर्ण

अटल व्योम multiq3_encoder_reset(काष्ठा comedi_device *dev,
				  अचिन्हित पूर्णांक chan)
अणु
	multiq3_set_ctrl(dev, MULTIQ3_CTRL_EN | MULTIQ3_CTRL_E_CHAN(chan));
	outb(MULTIQ3_EFLAG_RESET, dev->iobase + MULTIQ3_ENC_CTRL_REG);
	outb(MULTIQ3_BP_RESET, dev->iobase + MULTIQ3_ENC_CTRL_REG);
	outb(MULTIQ3_CLOCK_DATA, dev->iobase + MULTIQ3_ENC_DATA_REG);
	outb(MULTIQ3_CLOCK_SETUP, dev->iobase + MULTIQ3_ENC_CTRL_REG);
	outb(MULTIQ3_INPUT_SETUP, dev->iobase + MULTIQ3_ENC_CTRL_REG);
	outb(MULTIQ3_QUAD_X4, dev->iobase + MULTIQ3_ENC_CTRL_REG);
	outb(MULTIQ3_CNTR_RESET, dev->iobase + MULTIQ3_ENC_CTRL_REG);
पूर्ण

अटल पूर्णांक multiq3_encoder_insn_config(काष्ठा comedi_device *dev,
				       काष्ठा comedi_subdevice *s,
				       काष्ठा comedi_insn *insn,
				       अचिन्हित पूर्णांक *data)
अणु
	अचिन्हित पूर्णांक chan = CR_CHAN(insn->chanspec);

	चयन (data[0]) अणु
	हाल INSN_CONFIG_RESET:
		multiq3_encoder_reset(dev, chan);
		अवरोध;
	शेष:
		वापस -EINVAL;
	पूर्ण

	वापस insn->n;
पूर्ण

अटल पूर्णांक multiq3_attach(काष्ठा comedi_device *dev,
			  काष्ठा comedi_devconfig *it)
अणु
	काष्ठा comedi_subdevice *s;
	पूर्णांक ret;
	पूर्णांक i;

	ret = comedi_request_region(dev, it->options[0], 0x10);
	अगर (ret)
		वापस ret;

	ret = comedi_alloc_subdevices(dev, 5);
	अगर (ret)
		वापस ret;

	/* Analog Input subdevice */
	s = &dev->subdevices[0];
	s->type		= COMEDI_SUBD_AI;
	s->subdev_flags	= SDF_READABLE | SDF_GROUND;
	s->n_chan	= 8;
	s->maxdata	= 0x1fff;
	s->range_table	= &range_bipolar5;
	s->insn_पढ़ो	= multiq3_ai_insn_पढ़ो;

	/* Analog Output subdevice */
	s = &dev->subdevices[1];
	s->type		= COMEDI_SUBD_AO;
	s->subdev_flags	= SDF_WRITABLE;
	s->n_chan	= 8;
	s->maxdata	= 0x0fff;
	s->range_table	= &range_bipolar5;
	s->insn_ग_लिखो	= multiq3_ao_insn_ग_लिखो;

	ret = comedi_alloc_subdev_पढ़ोback(s);
	अगर (ret)
		वापस ret;

	/* Digital Input subdevice */
	s = &dev->subdevices[2];
	s->type		= COMEDI_SUBD_DI;
	s->subdev_flags	= SDF_READABLE;
	s->n_chan	= 16;
	s->maxdata	= 1;
	s->range_table	= &range_digital;
	s->insn_bits	= multiq3_di_insn_bits;

	/* Digital Output subdevice */
	s = &dev->subdevices[3];
	s->type		= COMEDI_SUBD_DO;
	s->subdev_flags	= SDF_WRITABLE;
	s->n_chan	= 16;
	s->maxdata	= 1;
	s->range_table	= &range_digital;
	s->insn_bits	= multiq3_करो_insn_bits;

	/* Encoder (Counter) subdevice */
	s = &dev->subdevices[4];
	s->type		= COMEDI_SUBD_COUNTER;
	s->subdev_flags	= SDF_READABLE | SDF_LSAMPL;
	s->n_chan	= it->options[2] * 2;
	s->maxdata	= 0x00ffffff;
	s->range_table	= &range_unknown;
	s->insn_पढ़ो	= multiq3_encoder_insn_पढ़ो;
	s->insn_config	= multiq3_encoder_insn_config;

	क्रम (i = 0; i < s->n_chan; i++)
		multiq3_encoder_reset(dev, i);

	वापस 0;
पूर्ण

अटल काष्ठा comedi_driver multiq3_driver = अणु
	.driver_name	= "multiq3",
	.module		= THIS_MODULE,
	.attach		= multiq3_attach,
	.detach		= comedi_legacy_detach,
पूर्ण;
module_comedi_driver(multiq3_driver);

MODULE_AUTHOR("Comedi https://www.comedi.org");
MODULE_DESCRIPTION("Comedi driver for Quanser Consulting MultiQ-3 board");
MODULE_LICENSE("GPL");
