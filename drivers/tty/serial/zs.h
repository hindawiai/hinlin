...
/*
 * zs.h: Definitions क्रम the DECstation Z85C30 serial driver.
 *
 * Adapted from drivers/sbus/अक्षर/sunserial.h by Paul Mackerras.
 * Adapted from drivers/macपूर्णांकosh/macserial.h by Harald Koerfgen.
 *
 * Copyright (C) 1996 Paul Mackerras (Paul.Mackerras@cs.anu.edu.au)
 * Copyright (C) 1995 David S. Miller (davem@caip.rutgers.edu)
 * Copyright (C) 2004, 2005, 2007  Maciej W. Rozycki
 */
#अगर_अघोषित _SERIAL_ZS_H
#घोषणा _SERIAL_ZS_H

#अगर_घोषित __KERNEL__

#घोषणा ZS_NUM_REGS 16

/*
 * This is our पूर्णांकernal काष्ठाure क्रम each serial port's state.
 */
काष्ठा zs_port अणु
	काष्ठा zs_scc	*scc;			/* Containing SCC.  */
	काष्ठा uart_port port;			/* Underlying UART.  */

	पूर्णांक		clk_mode;		/* May be 1, 16, 32, or 64.  */

	अचिन्हित पूर्णांक	tty_अवरोध;		/* Set on BREAK condition.  */
	पूर्णांक		tx_stopped;		/* Output is suspended.  */

	अचिन्हित पूर्णांक	mctrl;			/* State of modem lines.  */
	u8		brk;			/* BREAK state from RR0.  */

	u8		regs[ZS_NUM_REGS];	/* Channel ग_लिखो रेजिस्टरs.  */
पूर्ण;

/*
 * Per-SCC state क्रम locking and the पूर्णांकerrupt handler.
 */
काष्ठा zs_scc अणु
	काष्ठा zs_port	zport[2];
	spinlock_t	zlock;
	atomic_t	irq_guard;
...
/*
 * Conversion routines to/from brg समय स्थिरants from/to bits per second.
 */
#घोषणा ZS_BRG_TO_BPS(brg, freq) ((freq) / 2 / ((brg) + 2))
#घोषणा ZS_BPS_TO_BRG(bps, freq) ((((freq) + (bps)) / (2 * (bps))) - 2)
...
#घोषणा RxINT_ALL	0x10	/* Int on all Rx Characters or error */
#घोषणा RxINT_ERR	0x18	/* Int on error only */
...
#घोषणा SOFTACK		0x20	/* Software Interrupt Acknowledge */
...
#घोषणा WR7P_EN		1	/* WR7 Prime SDLC Feature Enable */
...
#घोषणा FRM_ERR		0x40	/* CRC/Framing Error */
#घोषणा END_FR		0x80	/* End of Frame (SDLC) */

/* Read Register 2 (Interrupt Vector (WR2) -- channel A).  */

/* Read Register 2 (Modअगरied Interrupt Vector -- channel B).  */
