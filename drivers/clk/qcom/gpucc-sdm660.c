<शैली गुरु>
// SPDX-License-Identअगरier: GPL-2.0-only
/*
 * Copyright (c) 2020, The Linux Foundation. All rights reserved.
 * Copyright (c) 2020, AngeloGioacchino Del Regno
 *                     <angelogioacchino.delregno@soमुख्यline.org>
 */

#समावेश <linux/bitops.h>
#समावेश <linux/clk.h>
#समावेश <linux/clk-provider.h>
#समावेश <linux/err.h>
#समावेश <linux/kernel.h>
#समावेश <linux/module.h>
#समावेश <linux/platक्रमm_device.h>
#समावेश <linux/of.h>
#समावेश <linux/of_device.h>
#समावेश <linux/regmap.h>
#समावेश <linux/reset-controller.h>
#समावेश <dt-bindings/घड़ी/qcom,gpucc-sdm660.h>

#समावेश "clk-alpha-pll.h"
#समावेश "common.h"
#समावेश "clk-regmap.h"
#समावेश "clk-pll.h"
#समावेश "clk-rcg.h"
#समावेश "clk-branch.h"
#समावेश "gdsc.h"
#समावेश "reset.h"

क्रमागत अणु
	P_GPU_XO,
	P_CORE_BI_PLL_TEST_SE,
	P_GPLL0_OUT_MAIN,
	P_GPLL0_OUT_MAIN_DIV,
	P_GPU_PLL0_PLL_OUT_MAIN,
	P_GPU_PLL1_PLL_OUT_MAIN,
पूर्ण;

अटल काष्ठा clk_branch gpucc_cxo_clk = अणु
	.halt_reg = 0x1020,
	.clkr = अणु
		.enable_reg = 0x1020,
		.enable_mask = BIT(0),
		.hw.init = &(काष्ठा clk_init_data)अणु
			.name = "gpucc_cxo_clk",
			.parent_data = &(स्थिर काष्ठा clk_parent_data)अणु
				.fw_name = "xo",
				.name = "xo"
			पूर्ण,
			.num_parents = 1,
			.ops = &clk_branch2_ops,
			.flags = CLK_IS_CRITICAL,
		पूर्ण,
	पूर्ण,
पूर्ण;

अटल काष्ठा pll_vco gpu_vco[] = अणु
	अणु 1000000000, 2000000000, 0 पूर्ण,
	अणु 500000000,  1000000000, 2 पूर्ण,
	अणु 250000000,   500000000, 3 पूर्ण,
पूर्ण;

अटल काष्ठा clk_alpha_pll gpu_pll0_pll_out_मुख्य = अणु
	.offset = 0x0,
	.regs = clk_alpha_pll_regs[CLK_ALPHA_PLL_TYPE_DEFAULT],
	.vco_table = gpu_vco,
	.num_vco = ARRAY_SIZE(gpu_vco),
	.clkr.hw.init = &(काष्ठा clk_init_data)अणु
		.name = "gpu_pll0_pll_out_main",
		.parent_data =  &(स्थिर काष्ठा clk_parent_data)अणु
			.hw = &gpucc_cxo_clk.clkr.hw,
		पूर्ण,
		.num_parents = 1,
		.ops = &clk_alpha_pll_ops,
	पूर्ण,
पूर्ण;

अटल काष्ठा clk_alpha_pll gpu_pll1_pll_out_मुख्य = अणु
	.offset = 0x40,
	.regs = clk_alpha_pll_regs[CLK_ALPHA_PLL_TYPE_DEFAULT],
	.vco_table = gpu_vco,
	.num_vco = ARRAY_SIZE(gpu_vco),
	.clkr.hw.init = &(काष्ठा clk_init_data)अणु
		.name = "gpu_pll1_pll_out_main",
		.parent_data = &(स्थिर काष्ठा clk_parent_data)अणु
			.hw = &gpucc_cxo_clk.clkr.hw,
		पूर्ण,
		.num_parents = 1,
		.ops = &clk_alpha_pll_ops,
	पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा parent_map gpucc_parent_map_1[] = अणु
	अणु P_GPU_XO, 0 पूर्ण,
	अणु P_GPU_PLL0_PLL_OUT_MAIN, 1 पूर्ण,
	अणु P_GPU_PLL1_PLL_OUT_MAIN, 3 पूर्ण,
	अणु P_GPLL0_OUT_MAIN, 5 पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा clk_parent_data gpucc_parent_data_1[] = अणु
	अणु .hw = &gpucc_cxo_clk.clkr.hw पूर्ण,
	अणु .hw = &gpu_pll0_pll_out_मुख्य.clkr.hw पूर्ण,
	अणु .hw = &gpu_pll1_pll_out_मुख्य.clkr.hw पूर्ण,
	अणु .fw_name = "gcc_gpu_gpll0_clk", .name = "gcc_gpu_gpll0_clk" पूर्ण,
पूर्ण;

अटल काष्ठा clk_rcg2_gfx3d gfx3d_clk_src = अणु
	.भाग = 2,
	.rcg = अणु
		.cmd_rcgr = 0x1070,
		.mnd_width = 0,
		.hid_width = 5,
		.parent_map = gpucc_parent_map_1,
		.clkr.hw.init = &(काष्ठा clk_init_data)अणु
			.name = "gfx3d_clk_src",
			.parent_data = gpucc_parent_data_1,
			.num_parents = 4,
			.ops = &clk_gfx3d_ops,
			.flags = CLK_SET_RATE_PARENT | CLK_OPS_PARENT_ENABLE,
		पूर्ण,
	पूर्ण,
	.hws = (काष्ठा clk_hw*[])अणु
		&gpucc_cxo_clk.clkr.hw,
		&gpu_pll0_pll_out_मुख्य.clkr.hw,
		&gpu_pll1_pll_out_मुख्य.clkr.hw,
	पूर्ण
पूर्ण;

अटल काष्ठा clk_branch gpucc_gfx3d_clk = अणु
	.halt_reg = 0x1098,
	.halt_check = BRANCH_HALT,
	.hwcg_reg = 0x1098,
	.hwcg_bit = 1,
	.clkr = अणु
		.enable_reg = 0x1098,
		.enable_mask = BIT(0),
		.hw.init = &(काष्ठा clk_init_data)अणु
			.name = "gpucc_gfx3d_clk",
			.parent_data = &(स्थिर काष्ठा clk_parent_data)अणु
				.hw = &gfx3d_clk_src.rcg.clkr.hw,
			पूर्ण,
			.num_parents = 1,
			.ops = &clk_branch2_ops,
			.flags = CLK_SET_RATE_PARENT,
		पूर्ण,
	पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा parent_map gpucc_parent_map_0[] = अणु
	अणु P_GPU_XO, 0 पूर्ण,
	अणु P_GPLL0_OUT_MAIN, 5 पूर्ण,
	अणु P_GPLL0_OUT_MAIN_DIV, 6 पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा clk_parent_data gpucc_parent_data_0[] = अणु
	अणु .hw = &gpucc_cxo_clk.clkr.hw पूर्ण,
	अणु .fw_name = "gcc_gpu_gpll0_clk", .name = "gcc_gpu_gpll0_clk" पूर्ण,
	अणु .fw_name = "gcc_gpu_gpll0_div_clk", .name = "gcc_gpu_gpll0_div_clk" पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा freq_tbl ftbl_rbbmसमयr_clk_src[] = अणु
	F(19200000, P_GPU_XO, 1, 0, 0),
	अणु पूर्ण
पूर्ण;

अटल काष्ठा clk_rcg2 rbbmसमयr_clk_src = अणु
	.cmd_rcgr = 0x10b0,
	.mnd_width = 0,
	.hid_width = 5,
	.parent_map = gpucc_parent_map_0,
	.freq_tbl = ftbl_rbbmसमयr_clk_src,
	.clkr.hw.init = &(काष्ठा clk_init_data)अणु
		.name = "rbbmtimer_clk_src",
		.parent_data = gpucc_parent_data_0,
		.num_parents = 3,
		.ops = &clk_rcg2_ops,
	पूर्ण,
पूर्ण;

अटल स्थिर काष्ठा freq_tbl ftbl_rbcpr_clk_src[] = अणु
	F(19200000, P_GPU_XO, 1, 0, 0),
	F(50000000, P_GPLL0_OUT_MAIN_DIV, 6, 0, 0),
	अणु पूर्ण
पूर्ण;

अटल काष्ठा clk_rcg2 rbcpr_clk_src = अणु
	.cmd_rcgr = 0x1030,
	.mnd_width = 0,
	.hid_width = 5,
	.parent_map = gpucc_parent_map_0,
	.freq_tbl = ftbl_rbcpr_clk_src,
	.clkr.hw.init = &(काष्ठा clk_init_data)अणु
		.name = "rbcpr_clk_src",
		.parent_data = gpucc_parent_data_0,
		.num_parents = 3,
		.ops = &clk_rcg2_ops,
	पूर्ण,
पूर्ण;

अटल काष्ठा clk_branch gpucc_rbbmसमयr_clk = अणु
	.halt_reg = 0x10d0,
	.halt_check = BRANCH_HALT,
	.clkr = अणु
		.enable_reg = 0x10d0,
		.enable_mask = BIT(0),
		.hw.init = &(काष्ठा clk_init_data)अणु
			.name = "gpucc_rbbmtimer_clk",
			.parent_names = (स्थिर अक्षर *[])अणु
				"rbbmtimer_clk_src",
			पूर्ण,
			.num_parents = 1,
			.flags = CLK_SET_RATE_PARENT,
			.ops = &clk_branch2_ops,
		पूर्ण,
	पूर्ण,
पूर्ण;

अटल काष्ठा clk_branch gpucc_rbcpr_clk = अणु
	.halt_reg = 0x1054,
	.halt_check = BRANCH_HALT,
	.clkr = अणु
		.enable_reg = 0x1054,
		.enable_mask = BIT(0),
		.hw.init = &(काष्ठा clk_init_data)अणु
			.name = "gpucc_rbcpr_clk",
			.parent_names = (स्थिर अक्षर *[])अणु
				"rbcpr_clk_src",
			पूर्ण,
			.num_parents = 1,
			.flags = CLK_SET_RATE_PARENT,
			.ops = &clk_branch2_ops,
		पूर्ण,
	पूर्ण,
पूर्ण;

अटल काष्ठा gdsc gpu_cx_gdsc = अणु
	.gdscr = 0x1004,
	.gds_hw_ctrl = 0x1008,
	.pd = अणु
		.name = "gpu_cx",
	पूर्ण,
	.pwrsts = PWRSTS_OFF_ON,
	.flags = VOTABLE,
पूर्ण;

अटल काष्ठा gdsc gpu_gx_gdsc = अणु
	.gdscr = 0x1094,
	.clamp_io_ctrl = 0x130,
	.resets = (अचिन्हित पूर्णांक [])अणु GPU_GX_BCR पूर्ण,
	.reset_count = 1,
	.cxcs = (अचिन्हित पूर्णांक [])अणु 0x1098 पूर्ण,
	.cxc_count = 1,
	.pd = अणु
		.name = "gpu_gx",
	पूर्ण,
	.parent = &gpu_cx_gdsc.pd,
	.pwrsts = PWRSTS_OFF | PWRSTS_ON | PWRSTS_RET,
	.flags = CLAMP_IO | SW_RESET | AON_RESET | NO_RET_PERIPH,
पूर्ण;

अटल काष्ठा gdsc *gpucc_sdm660_gdscs[] = अणु
	[GPU_CX_GDSC] = &gpu_cx_gdsc,
	[GPU_GX_GDSC] = &gpu_gx_gdsc,
पूर्ण;

अटल स्थिर काष्ठा qcom_reset_map gpucc_sdm660_resets[] = अणु
	[GPU_CX_BCR] = अणु 0x1000 पूर्ण,
	[RBCPR_BCR] = अणु 0x1050 पूर्ण,
	[GPU_GX_BCR] = अणु 0x1090 पूर्ण,
	[SPDM_BCR] = अणु 0x10E0 पूर्ण,
पूर्ण;

अटल काष्ठा clk_regmap *gpucc_sdm660_घड़ीs[] = अणु
	[GPUCC_CXO_CLK] = &gpucc_cxo_clk.clkr,
	[GPU_PLL0_PLL] = &gpu_pll0_pll_out_मुख्य.clkr,
	[GPU_PLL1_PLL] = &gpu_pll1_pll_out_मुख्य.clkr,
	[GFX3D_CLK_SRC] = &gfx3d_clk_src.rcg.clkr,
	[RBCPR_CLK_SRC] = &rbcpr_clk_src.clkr,
	[RBBMTIMER_CLK_SRC] = &rbbmसमयr_clk_src.clkr,
	[GPUCC_RBCPR_CLK] = &gpucc_rbcpr_clk.clkr,
	[GPUCC_GFX3D_CLK] = &gpucc_gfx3d_clk.clkr,
	[GPUCC_RBBMTIMER_CLK] = &gpucc_rbbmसमयr_clk.clkr,
पूर्ण;

अटल स्थिर काष्ठा regmap_config gpucc_660_regmap_config = अणु
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
	.max_रेजिस्टर	= 0x9034,
	.fast_io	= true,
पूर्ण;

अटल स्थिर काष्ठा qcom_cc_desc gpucc_sdm660_desc = अणु
	.config = &gpucc_660_regmap_config,
	.clks = gpucc_sdm660_घड़ीs,
	.num_clks = ARRAY_SIZE(gpucc_sdm660_घड़ीs),
	.resets = gpucc_sdm660_resets,
	.num_resets = ARRAY_SIZE(gpucc_sdm660_resets),
	.gdscs = gpucc_sdm660_gdscs,
	.num_gdscs = ARRAY_SIZE(gpucc_sdm660_gdscs),
पूर्ण;

अटल स्थिर काष्ठा of_device_id gpucc_sdm660_match_table[] = अणु
	अणु .compatible = "qcom,gpucc-sdm660" पूर्ण,
	अणु .compatible = "qcom,gpucc-sdm630" पूर्ण,
	अणु पूर्ण
पूर्ण;
MODULE_DEVICE_TABLE(of, gpucc_sdm660_match_table);

अटल पूर्णांक gpucc_sdm660_probe(काष्ठा platक्रमm_device *pdev)
अणु
	काष्ठा regmap *regmap;
	काष्ठा alpha_pll_config gpu_pll_config = अणु
		.config_ctl_val = 0x4001055b,
		.alpha = 0xaaaaab00,
		.alpha_en_mask = BIT(24),
		.vco_val = 0x2 << 20,
		.vco_mask = 0x3 << 20,
		.मुख्य_output_mask = 0x1,
	पूर्ण;

	regmap = qcom_cc_map(pdev, &gpucc_sdm660_desc);
	अगर (IS_ERR(regmap))
		वापस PTR_ERR(regmap);

	/* 800MHz configuration क्रम GPU PLL0 */
	gpu_pll_config.l = 0x29;
	gpu_pll_config.alpha_hi = 0xaa;
	clk_alpha_pll_configure(&gpu_pll0_pll_out_मुख्य, regmap, &gpu_pll_config);

	/* 740MHz configuration क्रम GPU PLL1 */
	gpu_pll_config.l = 0x26;
	gpu_pll_config.alpha_hi = 0x8a;
	clk_alpha_pll_configure(&gpu_pll1_pll_out_मुख्य, regmap, &gpu_pll_config);

	वापस qcom_cc_really_probe(pdev, &gpucc_sdm660_desc, regmap);
पूर्ण

अटल काष्ठा platक्रमm_driver gpucc_sdm660_driver = अणु
	.probe		= gpucc_sdm660_probe,
	.driver		= अणु
		.name	= "gpucc-sdm660",
		.of_match_table = gpucc_sdm660_match_table,
	पूर्ण,
पूर्ण;
module_platक्रमm_driver(gpucc_sdm660_driver);

MODULE_DESCRIPTION("Qualcomm SDM630/SDM660 GPUCC Driver");
MODULE_LICENSE("GPL v2");
