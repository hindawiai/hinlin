...
व्योम __init pxa168_clk_init(phys_addr_t mpmu_phys, phys_addr_t apmu_phys,
...
अणु
	...
	clk = mmp_clk_रेजिस्टर_apbc("ssp0", "ssp0_mux", apbc_base + APBC_SSP0,
				10, 0, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apbc("ssp1", "ssp1_mux", apbc_base + APBC_SSP1,
				10, 0, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apbc("ssp2", "ssp1_mux", apbc_base + APBC_SSP2,
				10, 0, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apbc("ssp3", "ssp1_mux", apbc_base + APBC_SSP3,
				10, 0, &clk_lock);
	clk_रेजिस्टर_clkdev(clk, शून्य, "mmp-ssp.3");

	clk = clk_रेजिस्टर_mux(शून्य, "ssp4_mux", ssp_parent,
				ARRAY_SIZE(ssp_parent),
				CLK_SET_RATE_PARENT | CLK_SET_RATE_NO_REPARENT,
				apbc_base + APBC_SSP4, 4, 3, 0, &clk_lock);
	clk_रेजिस्टर_clkdev(clk, "ssp_mux.4", शून्य);

	clk = mmp_clk_रेजिस्टर_apbc("ssp4", "ssp1_mux", apbc_base + APBC_SSP4,
				10, 0, &clk_lock);
	clk_रेजिस्टर_clkdev(clk, शून्य, "mmp-ssp.4");

	clk = mmp_clk_रेजिस्टर_apmu("dfc", "pll1_4", apmu_base + APMU_DFC,
				0x19b, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apmu("sdh0", "sdh_mux", apmu_base + APMU_SDH0,
				0x1b, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apmu("sdh1", "sdh1_mux", apmu_base + APMU_SDH1,
				0x1b, &clk_lock);
	...
	clk = mmp_clk_रेजिस्टर_apmu("sph", "usb_pll", apmu_base + APMU_USB,
				0x12, &clk_lock);
	...
	clk_रेजिस्टर_clkdev(clk, "fnclk", "mmp-disp.0");

	clk = mmp_clk_रेजिस्टर_apmu("disp0_hclk", "disp0_mux",
				apmu_base + APMU_DISP0, 0x24, &clk_lock);
	clk_रेजिस्टर_clkdev(clk, "hclk", "mmp-disp.0");
