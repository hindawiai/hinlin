<शैली गुरु>
/* SPDX-License-Identअगरier: GPL-2.0 */
/* Copyright(c) 1999 - 2018 Intel Corporation. */

#अगर_अघोषित _E1000E_PHY_H_
#घोषणा _E1000E_PHY_H_

s32 e1000e_check_करोwnshअगरt(काष्ठा e1000_hw *hw);
s32 e1000_check_polarity_m88(काष्ठा e1000_hw *hw);
s32 e1000_check_polarity_igp(काष्ठा e1000_hw *hw);
s32 e1000_check_polarity_अगरe(काष्ठा e1000_hw *hw);
s32 e1000e_check_reset_block_generic(काष्ठा e1000_hw *hw);
s32 e1000e_copper_link_setup_igp(काष्ठा e1000_hw *hw);
s32 e1000e_copper_link_setup_m88(काष्ठा e1000_hw *hw);
s32 e1000e_phy_क्रमce_speed_duplex_igp(काष्ठा e1000_hw *hw);
s32 e1000e_phy_क्रमce_speed_duplex_m88(काष्ठा e1000_hw *hw);
s32 e1000_phy_क्रमce_speed_duplex_अगरe(काष्ठा e1000_hw *hw);
s32 e1000e_get_cable_length_m88(काष्ठा e1000_hw *hw);
s32 e1000e_get_cable_length_igp_2(काष्ठा e1000_hw *hw);
s32 e1000e_get_cfg_करोne_generic(काष्ठा e1000_hw *hw);
s32 e1000e_get_phy_id(काष्ठा e1000_hw *hw);
s32 e1000e_get_phy_info_igp(काष्ठा e1000_hw *hw);
s32 e1000e_get_phy_info_m88(काष्ठा e1000_hw *hw);
s32 e1000_get_phy_info_अगरe(काष्ठा e1000_hw *hw);
s32 e1000e_phy_sw_reset(काष्ठा e1000_hw *hw);
व्योम e1000e_phy_क्रमce_speed_duplex_setup(काष्ठा e1000_hw *hw, u16 *phy_ctrl);
s32 e1000e_phy_hw_reset_generic(काष्ठा e1000_hw *hw);
s32 e1000e_phy_reset_dsp(काष्ठा e1000_hw *hw);
s32 e1000e_पढ़ो_kmrn_reg(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_पढ़ो_kmrn_reg_locked(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000_set_page_igp(काष्ठा e1000_hw *hw, u16 page);
s32 e1000e_पढ़ो_phy_reg_igp(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_पढ़ो_phy_reg_igp_locked(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_पढ़ो_phy_reg_m88(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_set_d3_lplu_state(काष्ठा e1000_hw *hw, bool active);
s32 e1000e_setup_copper_link(काष्ठा e1000_hw *hw);
s32 e1000e_ग_लिखो_kmrn_reg(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_ग_लिखो_kmrn_reg_locked(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_ग_लिखो_phy_reg_igp(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_ग_लिखो_phy_reg_igp_locked(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_ग_लिखो_phy_reg_m88(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_phy_has_link_generic(काष्ठा e1000_hw *hw, u32 iterations,
				u32 usec_पूर्णांकerval, bool *success);
s32 e1000e_phy_init_script_igp3(काष्ठा e1000_hw *hw);
क्रमागत e1000_phy_type e1000e_get_phy_type_from_id(u32 phy_id);
s32 e1000e_determine_phy_address(काष्ठा e1000_hw *hw);
s32 e1000e_ग_लिखो_phy_reg_bm(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000e_पढ़ो_phy_reg_bm(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000_enable_phy_wakeup_reg_access_bm(काष्ठा e1000_hw *hw, u16 *phy_reg);
s32 e1000_disable_phy_wakeup_reg_access_bm(काष्ठा e1000_hw *hw, u16 *phy_reg);
s32 e1000e_पढ़ो_phy_reg_bm2(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_ग_लिखो_phy_reg_bm2(काष्ठा e1000_hw *hw, u32 offset, u16 data);
व्योम e1000_घातer_up_phy_copper(काष्ठा e1000_hw *hw);
व्योम e1000_घातer_करोwn_phy_copper(काष्ठा e1000_hw *hw);
s32 e1000e_पढ़ो_phy_reg_mdic(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000e_ग_लिखो_phy_reg_mdic(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000_पढ़ो_phy_reg_hv(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000_पढ़ो_phy_reg_hv_locked(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000_पढ़ो_phy_reg_page_hv(काष्ठा e1000_hw *hw, u32 offset, u16 *data);
s32 e1000_ग_लिखो_phy_reg_hv(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000_ग_लिखो_phy_reg_hv_locked(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000_ग_लिखो_phy_reg_page_hv(काष्ठा e1000_hw *hw, u32 offset, u16 data);
s32 e1000_link_stall_workaround_hv(काष्ठा e1000_hw *hw);
s32 e1000_copper_link_setup_82577(काष्ठा e1000_hw *hw);
s32 e1000_check_polarity_82577(काष्ठा e1000_hw *hw);
s32 e1000_get_phy_info_82577(काष्ठा e1000_hw *hw);
s32 e1000_phy_क्रमce_speed_duplex_82577(काष्ठा e1000_hw *hw);
s32 e1000_get_cable_length_82577(काष्ठा e1000_hw *hw);

#घोषणा E1000_MAX_PHY_ADDR		8

/* IGP01E1000 Specअगरic Registers */
#घोषणा IGP01E1000_PHY_PORT_CONFIG	0x10	/* Port Config */
#घोषणा IGP01E1000_PHY_PORT_STATUS	0x11	/* Status */
#घोषणा IGP01E1000_PHY_PORT_CTRL	0x12	/* Control */
#घोषणा IGP01E1000_PHY_LINK_HEALTH	0x13	/* PHY Link Health */
#घोषणा IGP02E1000_PHY_POWER_MGMT	0x19	/* Power Management */
#घोषणा IGP01E1000_PHY_PAGE_SELECT	0x1F	/* Page Select */
#घोषणा BM_PHY_PAGE_SELECT		22	/* Page Select क्रम BM */
#घोषणा IGP_PAGE_SHIFT			5
#घोषणा PHY_REG_MASK			0x1F

/* BM/HV Specअगरic Registers */
#घोषणा BM_PORT_CTRL_PAGE		769
#घोषणा BM_WUC_PAGE			800
#घोषणा BM_WUC_ADDRESS_OPCODE		0x11
#घोषणा BM_WUC_DATA_OPCODE		0x12
#घोषणा BM_WUC_ENABLE_PAGE		BM_PORT_CTRL_PAGE
#घोषणा BM_WUC_ENABLE_REG		17
#घोषणा BM_WUC_ENABLE_BIT		BIT(2)
#घोषणा BM_WUC_HOST_WU_BIT		BIT(4)
#घोषणा BM_WUC_ME_WU_BIT		BIT(5)

#घोषणा PHY_UPPER_SHIFT			21
#घोषणा BM_PHY_REG(page, reg) \
	(((reg) & MAX_PHY_REG_ADDRESS) |\
	 (((page) & 0xFFFF) << PHY_PAGE_SHIFT) |\
	 (((reg) & ~MAX_PHY_REG_ADDRESS) << (PHY_UPPER_SHIFT - PHY_PAGE_SHIFT)))
#घोषणा BM_PHY_REG_PAGE(offset) \
	((u16)(((offset) >> PHY_PAGE_SHIFT) & 0xFFFF))
#घोषणा BM_PHY_REG_NUM(offset) \
	((u16)(((offset) & MAX_PHY_REG_ADDRESS) |\
	 (((offset) >> (PHY_UPPER_SHIFT - PHY_PAGE_SHIFT)) &\
		~MAX_PHY_REG_ADDRESS)))

#घोषणा HV_INTC_FC_PAGE_START		768
#घोषणा I82578_ADDR_REG			29
#घोषणा I82577_ADDR_REG			16
#घोषणा I82577_CFG_REG			22
#घोषणा I82577_CFG_ASSERT_CRS_ON_TX	BIT(15)
#घोषणा I82577_CFG_ENABLE_DOWNSHIFT	(3u << 10)	/* स्वतः करोwnshअगरt */
#घोषणा I82577_CTRL_REG			23

/* 82577 specअगरic PHY रेजिस्टरs */
#घोषणा I82577_PHY_CTRL_2		18
#घोषणा I82577_PHY_LBK_CTRL		19
#घोषणा I82577_PHY_STATUS_2		26
#घोषणा I82577_PHY_DIAG_STATUS		31

/* I82577 PHY Status 2 */
#घोषणा I82577_PHY_STATUS2_REV_POLARITY		0x0400
#घोषणा I82577_PHY_STATUS2_MDIX			0x0800
#घोषणा I82577_PHY_STATUS2_SPEED_MASK		0x0300
#घोषणा I82577_PHY_STATUS2_SPEED_1000MBPS	0x0200

/* I82577 PHY Control 2 */
#घोषणा I82577_PHY_CTRL2_MANUAL_MDIX		0x0200
#घोषणा I82577_PHY_CTRL2_AUTO_MDI_MDIX		0x0400
#घोषणा I82577_PHY_CTRL2_MDIX_CFG_MASK		0x0600

/* I82577 PHY Diagnostics Status */
#घोषणा I82577_DSTATUS_CABLE_LENGTH		0x03FC
#घोषणा I82577_DSTATUS_CABLE_LENGTH_SHIFT	2

/* BM PHY Copper Specअगरic Control 1 */
#घोषणा BM_CS_CTRL1			16

/* BM PHY Copper Specअगरic Status */
#घोषणा BM_CS_STATUS			17
#घोषणा BM_CS_STATUS_LINK_UP		0x0400
#घोषणा BM_CS_STATUS_RESOLVED		0x0800
#घोषणा BM_CS_STATUS_SPEED_MASK		0xC000
#घोषणा BM_CS_STATUS_SPEED_1000		0x8000

/* 82577 Mobile Phy Status Register */
#घोषणा HV_M_STATUS			26
#घोषणा HV_M_STATUS_AUTONEG_COMPLETE	0x1000
#घोषणा HV_M_STATUS_SPEED_MASK		0x0300
#घोषणा HV_M_STATUS_SPEED_1000		0x0200
#घोषणा HV_M_STATUS_SPEED_100		0x0100
#घोषणा HV_M_STATUS_LINK_UP		0x0040

#घोषणा IGP01E1000_PHY_PCS_INIT_REG	0x00B4
#घोषणा IGP01E1000_PHY_POLARITY_MASK	0x0078

#घोषणा IGP01E1000_PSCR_AUTO_MDIX	0x1000
#घोषणा IGP01E1000_PSCR_FORCE_MDI_MDIX	0x2000	/* 0=MDI, 1=MDIX */

#घोषणा IGP01E1000_PSCFR_SMART_SPEED	0x0080

#घोषणा IGP02E1000_PM_SPD		0x0001	/* Smart Power Down */
#घोषणा IGP02E1000_PM_D0_LPLU		0x0002	/* For D0a states */
#घोषणा IGP02E1000_PM_D3_LPLU		0x0004	/* For all other states */

#घोषणा IGP01E1000_PLHR_SS_DOWNGRADE	0x8000

#घोषणा IGP01E1000_PSSR_POLARITY_REVERSED	0x0002
#घोषणा IGP01E1000_PSSR_MDIX		0x0800
#घोषणा IGP01E1000_PSSR_SPEED_MASK	0xC000
#घोषणा IGP01E1000_PSSR_SPEED_1000MBPS	0xC000

#घोषणा IGP02E1000_PHY_CHANNEL_NUM	4
#घोषणा IGP02E1000_PHY_AGC_A		0x11B1
#घोषणा IGP02E1000_PHY_AGC_B		0x12B1
#घोषणा IGP02E1000_PHY_AGC_C		0x14B1
#घोषणा IGP02E1000_PHY_AGC_D		0x18B1

#घोषणा IGP02E1000_AGC_LENGTH_SHIFT	9	/* Course=15:13, Fine=12:9 */
#घोषणा IGP02E1000_AGC_LENGTH_MASK	0x7F
#घोषणा IGP02E1000_AGC_RANGE		15

#घोषणा E1000_CABLE_LENGTH_UNDEFINED	0xFF

#घोषणा E1000_KMRNCTRLSTA_OFFSET	0x001F0000
#घोषणा E1000_KMRNCTRLSTA_OFFSET_SHIFT	16
#घोषणा E1000_KMRNCTRLSTA_REN		0x00200000
#घोषणा E1000_KMRNCTRLSTA_CTRL_OFFSET	0x1	/* Kumeran Control */
#घोषणा E1000_KMRNCTRLSTA_DIAG_OFFSET	0x3	/* Kumeran Diagnostic */
#घोषणा E1000_KMRNCTRLSTA_TIMEOUTS	0x4	/* Kumeran Timeouts */
#घोषणा E1000_KMRNCTRLSTA_INBAND_PARAM	0x9	/* Kumeran InBand Parameters */
#घोषणा E1000_KMRNCTRLSTA_IBIST_DISABLE	0x0200	/* Kumeran IBIST Disable */
#घोषणा E1000_KMRNCTRLSTA_DIAG_NELPBK	0x1000	/* Nearend Loopback mode */
#घोषणा E1000_KMRNCTRLSTA_K1_CONFIG	0x7
#घोषणा E1000_KMRNCTRLSTA_K1_ENABLE	0x0002	/* enable K1 */
#घोषणा E1000_KMRNCTRLSTA_HD_CTRL	0x10	/* Kumeran HD Control */

#घोषणा IFE_PHY_EXTENDED_STATUS_CONTROL	0x10
#घोषणा IFE_PHY_SPECIAL_CONTROL		0x11	/* 100BaseTx PHY Special Ctrl */
#घोषणा IFE_PHY_SPECIAL_CONTROL_LED	0x1B	/* PHY Special and LED Ctrl */
#घोषणा IFE_PHY_MDIX_CONTROL		0x1C	/* MDI/MDI-X Control */

/* IFE PHY Extended Status Control */
#घोषणा IFE_PESC_POLARITY_REVERSED	0x0100

/* IFE PHY Special Control */
#घोषणा IFE_PSC_AUTO_POLARITY_DISABLE	0x0010
#घोषणा IFE_PSC_FORCE_POLARITY		0x0020

/* IFE PHY Special Control and LED Control */
#घोषणा IFE_PSCL_PROBE_MODE		0x0020
#घोषणा IFE_PSCL_PROBE_LEDS_OFF		0x0006	/* Force LEDs 0 and 2 off */
#घोषणा IFE_PSCL_PROBE_LEDS_ON		0x0007	/* Force LEDs 0 and 2 on */

/* IFE PHY MDIX Control */
#घोषणा IFE_PMC_MDIX_STATUS		0x0020	/* 1=MDI-X, 0=MDI */
#घोषणा IFE_PMC_FORCE_MDIX		0x0040	/* 1=क्रमce MDI-X, 0=क्रमce MDI */
#घोषणा IFE_PMC_AUTO_MDIX		0x0080	/* 1=enable स्वतः, 0=disable */

#पूर्ण_अगर
