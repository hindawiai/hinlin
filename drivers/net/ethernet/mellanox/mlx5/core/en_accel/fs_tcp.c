<शैली गुरु>
// SPDX-License-Identअगरier: GPL-2.0 OR Linux-OpenIB
/* Copyright (c) 2020, Mellanox Technologies inc. All rights reserved. */

#समावेश <linux/netdevice.h>
#समावेश "en_accel/fs_tcp.h"
#समावेश "fs_core.h"

क्रमागत accel_fs_tcp_type अणु
	ACCEL_FS_IPV4_TCP,
	ACCEL_FS_IPV6_TCP,
	ACCEL_FS_TCP_NUM_TYPES,
पूर्ण;

काष्ठा mlx5e_accel_fs_tcp अणु
	काष्ठा mlx5e_flow_table tables[ACCEL_FS_TCP_NUM_TYPES];
	काष्ठा mlx5_flow_handle *शेष_rules[ACCEL_FS_TCP_NUM_TYPES];
पूर्ण;

अटल क्रमागत mlx5e_traffic_types fs_accel2tt(क्रमागत accel_fs_tcp_type i)
अणु
	चयन (i) अणु
	हाल ACCEL_FS_IPV4_TCP:
		वापस MLX5E_TT_IPV4_TCP;
	शेष: /* ACCEL_FS_IPV6_TCP */
		वापस MLX5E_TT_IPV6_TCP;
	पूर्ण
पूर्ण

अटल व्योम accel_fs_tcp_set_ipv4_flow(काष्ठा mlx5_flow_spec *spec, काष्ठा sock *sk)
अणु
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria, outer_headers.ip_protocol);
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.ip_protocol, IPPROTO_TCP);
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria, outer_headers.ip_version);
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.ip_version, 4);
	स_नकल(MLX5_ADDR_OF(fte_match_param, spec->match_value,
			    outer_headers.src_ipv4_src_ipv6.ipv4_layout.ipv4),
	       &inet_sk(sk)->inet_daddr, 4);
	स_नकल(MLX5_ADDR_OF(fte_match_param, spec->match_value,
			    outer_headers.dst_ipv4_dst_ipv6.ipv4_layout.ipv4),
	       &inet_sk(sk)->inet_rcv_saddr, 4);
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
			 outer_headers.src_ipv4_src_ipv6.ipv4_layout.ipv4);
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
			 outer_headers.dst_ipv4_dst_ipv6.ipv4_layout.ipv4);
पूर्ण

#अगर IS_ENABLED(CONFIG_IPV6)
अटल व्योम accel_fs_tcp_set_ipv6_flow(काष्ठा mlx5_flow_spec *spec, काष्ठा sock *sk)
अणु
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria, outer_headers.ip_protocol);
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.ip_protocol, IPPROTO_TCP);
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria, outer_headers.ip_version);
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.ip_version, 6);
	स_नकल(MLX5_ADDR_OF(fte_match_param, spec->match_value,
			    outer_headers.src_ipv4_src_ipv6.ipv6_layout.ipv6),
	       &sk->sk_v6_daddr, 16);
	स_नकल(MLX5_ADDR_OF(fte_match_param, spec->match_value,
			    outer_headers.dst_ipv4_dst_ipv6.ipv6_layout.ipv6),
	       &inet6_sk(sk)->saddr, 16);
	स_रखो(MLX5_ADDR_OF(fte_match_param, spec->match_criteria,
			    outer_headers.src_ipv4_src_ipv6.ipv6_layout.ipv6),
	       0xff, 16);
	स_रखो(MLX5_ADDR_OF(fte_match_param, spec->match_criteria,
			    outer_headers.dst_ipv4_dst_ipv6.ipv6_layout.ipv6),
	       0xff, 16);
पूर्ण
#पूर्ण_अगर

व्योम mlx5e_accel_fs_del_sk(काष्ठा mlx5_flow_handle *rule)
अणु
	mlx5_del_flow_rules(rule);
पूर्ण

काष्ठा mlx5_flow_handle *mlx5e_accel_fs_add_sk(काष्ठा mlx5e_priv *priv,
					       काष्ठा sock *sk, u32 tirn,
					       uपूर्णांक32_t flow_tag)
अणु
	काष्ठा mlx5_flow_destination dest = अणुपूर्ण;
	काष्ठा mlx5e_flow_table *ft = शून्य;
	काष्ठा mlx5e_accel_fs_tcp *fs_tcp;
	MLX5_DECLARE_FLOW_ACT(flow_act);
	काष्ठा mlx5_flow_handle *flow;
	काष्ठा mlx5_flow_spec *spec;

	spec = kvzalloc(माप(*spec), GFP_KERNEL);
	अगर (!spec)
		वापस ERR_PTR(-ENOMEM);

	fs_tcp = priv->fs.accel_tcp;

	spec->match_criteria_enable = MLX5_MATCH_OUTER_HEADERS;

	चयन (sk->sk_family) अणु
	हाल AF_INET:
		accel_fs_tcp_set_ipv4_flow(spec, sk);
		ft = &fs_tcp->tables[ACCEL_FS_IPV4_TCP];
		mlx5e_dbg(HW, priv, "%s flow is %pI4:%d -> %pI4:%d\n", __func__,
			  &inet_sk(sk)->inet_rcv_saddr,
			  inet_sk(sk)->inet_sport,
			  &inet_sk(sk)->inet_daddr,
			  inet_sk(sk)->inet_dport);
		अवरोध;
#अगर IS_ENABLED(CONFIG_IPV6)
	हाल AF_INET6:
		अगर (!sk->sk_ipv6only &&
		    ipv6_addr_type(&sk->sk_v6_daddr) == IPV6_ADDR_MAPPED) अणु
			accel_fs_tcp_set_ipv4_flow(spec, sk);
			ft = &fs_tcp->tables[ACCEL_FS_IPV4_TCP];
		पूर्ण अन्यथा अणु
			accel_fs_tcp_set_ipv6_flow(spec, sk);
			ft = &fs_tcp->tables[ACCEL_FS_IPV6_TCP];
		पूर्ण
		अवरोध;
#पूर्ण_अगर
	शेष:
		अवरोध;
	पूर्ण

	अगर (!ft) अणु
		flow = ERR_PTR(-EINVAL);
		जाओ out;
	पूर्ण

	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
			 outer_headers.tcp_dport);
	MLX5_SET_TO_ONES(fte_match_param, spec->match_criteria,
			 outer_headers.tcp_sport);
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.tcp_dport,
		 ntohs(inet_sk(sk)->inet_sport));
	MLX5_SET(fte_match_param, spec->match_value, outer_headers.tcp_sport,
		 ntohs(inet_sk(sk)->inet_dport));

	dest.type = MLX5_FLOW_DESTINATION_TYPE_TIR;
	dest.tir_num = tirn;
	अगर (flow_tag != MLX5_FS_DEFAULT_FLOW_TAG) अणु
		spec->flow_context.flow_tag = flow_tag;
		spec->flow_context.flags = FLOW_CONTEXT_HAS_TAG;
	पूर्ण

	flow = mlx5_add_flow_rules(ft->t, spec, &flow_act, &dest, 1);

	अगर (IS_ERR(flow))
		netdev_err(priv->netdev, "mlx5_add_flow_rules() failed, flow is %ld\n",
			   PTR_ERR(flow));

out:
	kvमुक्त(spec);
	वापस flow;
पूर्ण

अटल पूर्णांक accel_fs_tcp_add_शेष_rule(काष्ठा mlx5e_priv *priv,
					 क्रमागत accel_fs_tcp_type type)
अणु
	काष्ठा mlx5e_flow_table *accel_fs_t;
	काष्ठा mlx5_flow_destination dest;
	काष्ठा mlx5e_accel_fs_tcp *fs_tcp;
	MLX5_DECLARE_FLOW_ACT(flow_act);
	काष्ठा mlx5_flow_handle *rule;
	पूर्णांक err = 0;

	fs_tcp = priv->fs.accel_tcp;
	accel_fs_t = &fs_tcp->tables[type];

	dest = mlx5e_ttc_get_शेष_dest(priv, fs_accel2tt(type));
	rule = mlx5_add_flow_rules(accel_fs_t->t, शून्य, &flow_act, &dest, 1);
	अगर (IS_ERR(rule)) अणु
		err = PTR_ERR(rule);
		netdev_err(priv->netdev,
			   "%s: add default rule failed, accel_fs type=%d, err %d\n",
			   __func__, type, err);
		वापस err;
	पूर्ण

	fs_tcp->शेष_rules[type] = rule;
	वापस 0;
पूर्ण

#घोषणा MLX5E_ACCEL_FS_TCP_NUM_GROUPS	(2)
#घोषणा MLX5E_ACCEL_FS_TCP_GROUP1_SIZE	(BIT(16) - 1)
#घोषणा MLX5E_ACCEL_FS_TCP_GROUP2_SIZE	(BIT(0))
#घोषणा MLX5E_ACCEL_FS_TCP_TABLE_SIZE	(MLX5E_ACCEL_FS_TCP_GROUP1_SIZE +\
					 MLX5E_ACCEL_FS_TCP_GROUP2_SIZE)
अटल पूर्णांक accel_fs_tcp_create_groups(काष्ठा mlx5e_flow_table *ft,
				      क्रमागत accel_fs_tcp_type type)
अणु
	पूर्णांक inlen = MLX5_ST_SZ_BYTES(create_flow_group_in);
	व्योम *outer_headers_c;
	पूर्णांक ix = 0;
	u32 *in;
	पूर्णांक err;
	u8 *mc;

	ft->g = kसुस्मृति(MLX5E_ACCEL_FS_TCP_NUM_GROUPS, माप(*ft->g), GFP_KERNEL);
	in = kvzalloc(inlen, GFP_KERNEL);
	अगर  (!in || !ft->g) अणु
		kमुक्त(ft->g);
		kvमुक्त(in);
		वापस -ENOMEM;
	पूर्ण

	mc = MLX5_ADDR_OF(create_flow_group_in, in, match_criteria);
	outer_headers_c = MLX5_ADDR_OF(fte_match_param, mc, outer_headers);
	MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c, ip_protocol);
	MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c, ip_version);

	चयन (type) अणु
	हाल ACCEL_FS_IPV4_TCP:
	हाल ACCEL_FS_IPV6_TCP:
		MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c, tcp_dport);
		MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c, tcp_sport);
		अवरोध;
	शेष:
		err = -EINVAL;
		जाओ out;
	पूर्ण

	चयन (type) अणु
	हाल ACCEL_FS_IPV4_TCP:
		MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c,
				 src_ipv4_src_ipv6.ipv4_layout.ipv4);
		MLX5_SET_TO_ONES(fte_match_set_lyr_2_4, outer_headers_c,
				 dst_ipv4_dst_ipv6.ipv4_layout.ipv4);
		अवरोध;
	हाल ACCEL_FS_IPV6_TCP:
		स_रखो(MLX5_ADDR_OF(fte_match_set_lyr_2_4, outer_headers_c,
				    src_ipv4_src_ipv6.ipv6_layout.ipv6),
		       0xff, 16);
		स_रखो(MLX5_ADDR_OF(fte_match_set_lyr_2_4, outer_headers_c,
				    dst_ipv4_dst_ipv6.ipv6_layout.ipv6),
		       0xff, 16);
		अवरोध;
	शेष:
		err = -EINVAL;
		जाओ out;
	पूर्ण

	MLX5_SET_CFG(in, match_criteria_enable, MLX5_MATCH_OUTER_HEADERS);
	MLX5_SET_CFG(in, start_flow_index, ix);
	ix += MLX5E_ACCEL_FS_TCP_GROUP1_SIZE;
	MLX5_SET_CFG(in, end_flow_index, ix - 1);
	ft->g[ft->num_groups] = mlx5_create_flow_group(ft->t, in);
	अगर (IS_ERR(ft->g[ft->num_groups]))
		जाओ err;
	ft->num_groups++;

	/* Default Flow Group */
	स_रखो(in, 0, inlen);
	MLX5_SET_CFG(in, start_flow_index, ix);
	ix += MLX5E_ACCEL_FS_TCP_GROUP2_SIZE;
	MLX5_SET_CFG(in, end_flow_index, ix - 1);
	ft->g[ft->num_groups] = mlx5_create_flow_group(ft->t, in);
	अगर (IS_ERR(ft->g[ft->num_groups]))
		जाओ err;
	ft->num_groups++;

	kvमुक्त(in);
	वापस 0;

err:
	err = PTR_ERR(ft->g[ft->num_groups]);
	ft->g[ft->num_groups] = शून्य;
out:
	kvमुक्त(in);

	वापस err;
पूर्ण

अटल पूर्णांक accel_fs_tcp_create_table(काष्ठा mlx5e_priv *priv, क्रमागत accel_fs_tcp_type type)
अणु
	काष्ठा mlx5e_flow_table *ft = &priv->fs.accel_tcp->tables[type];
	काष्ठा mlx5_flow_table_attr ft_attr = अणुपूर्ण;
	पूर्णांक err;

	ft->num_groups = 0;

	ft_attr.max_fte = MLX5E_ACCEL_FS_TCP_TABLE_SIZE;
	ft_attr.level = MLX5E_ACCEL_FS_TCP_FT_LEVEL;
	ft_attr.prio = MLX5E_NIC_PRIO;

	ft->t = mlx5_create_flow_table(priv->fs.ns, &ft_attr);
	अगर (IS_ERR(ft->t)) अणु
		err = PTR_ERR(ft->t);
		ft->t = शून्य;
		वापस err;
	पूर्ण

	netdev_dbg(priv->netdev, "Created fs accel table id %u level %u\n",
		   ft->t->id, ft->t->level);

	err = accel_fs_tcp_create_groups(ft, type);
	अगर (err)
		जाओ err;

	err = accel_fs_tcp_add_शेष_rule(priv, type);
	अगर (err)
		जाओ err;

	वापस 0;
err:
	mlx5e_destroy_flow_table(ft);
	वापस err;
पूर्ण

अटल पूर्णांक accel_fs_tcp_disable(काष्ठा mlx5e_priv *priv)
अणु
	पूर्णांक err, i;

	क्रम (i = 0; i < ACCEL_FS_TCP_NUM_TYPES; i++) अणु
		/* Modअगरy ttc rules destination to poपूर्णांक back to the indir TIRs */
		err = mlx5e_ttc_fwd_शेष_dest(priv, fs_accel2tt(i));
		अगर (err) अणु
			netdev_err(priv->netdev,
				   "%s: modify ttc[%d] default destination failed, err(%d)\n",
				   __func__, fs_accel2tt(i), err);
			वापस err;
		पूर्ण
	पूर्ण

	वापस 0;
पूर्ण

अटल पूर्णांक accel_fs_tcp_enable(काष्ठा mlx5e_priv *priv)
अणु
	काष्ठा mlx5_flow_destination dest = अणुपूर्ण;
	पूर्णांक err, i;

	dest.type = MLX5_FLOW_DESTINATION_TYPE_FLOW_TABLE;
	क्रम (i = 0; i < ACCEL_FS_TCP_NUM_TYPES; i++) अणु
		dest.ft = priv->fs.accel_tcp->tables[i].t;

		/* Modअगरy ttc rules destination to poपूर्णांक on the accel_fs FTs */
		err = mlx5e_ttc_fwd_dest(priv, fs_accel2tt(i), &dest);
		अगर (err) अणु
			netdev_err(priv->netdev,
				   "%s: modify ttc[%d] destination to accel failed, err(%d)\n",
				   __func__, fs_accel2tt(i), err);
			वापस err;
		पूर्ण
	पूर्ण
	वापस 0;
पूर्ण

अटल व्योम accel_fs_tcp_destroy_table(काष्ठा mlx5e_priv *priv, पूर्णांक i)
अणु
	काष्ठा mlx5e_accel_fs_tcp *fs_tcp;

	fs_tcp = priv->fs.accel_tcp;
	अगर (IS_ERR_OR_शून्य(fs_tcp->tables[i].t))
		वापस;

	mlx5_del_flow_rules(fs_tcp->शेष_rules[i]);
	mlx5e_destroy_flow_table(&fs_tcp->tables[i]);
	fs_tcp->tables[i].t = शून्य;
पूर्ण

व्योम mlx5e_accel_fs_tcp_destroy(काष्ठा mlx5e_priv *priv)
अणु
	पूर्णांक i;

	अगर (!priv->fs.accel_tcp)
		वापस;

	accel_fs_tcp_disable(priv);

	क्रम (i = 0; i < ACCEL_FS_TCP_NUM_TYPES; i++)
		accel_fs_tcp_destroy_table(priv, i);

	kमुक्त(priv->fs.accel_tcp);
	priv->fs.accel_tcp = शून्य;
पूर्ण

पूर्णांक mlx5e_accel_fs_tcp_create(काष्ठा mlx5e_priv *priv)
अणु
	पूर्णांक i, err;

	अगर (!MLX5_CAP_FLOWTABLE_NIC_RX(priv->mdev, ft_field_support.outer_ip_version))
		वापस -EOPNOTSUPP;

	priv->fs.accel_tcp = kzalloc(माप(*priv->fs.accel_tcp), GFP_KERNEL);
	अगर (!priv->fs.accel_tcp)
		वापस -ENOMEM;

	क्रम (i = 0; i < ACCEL_FS_TCP_NUM_TYPES; i++) अणु
		err = accel_fs_tcp_create_table(priv, i);
		अगर (err)
			जाओ err_destroy_tables;
	पूर्ण

	err = accel_fs_tcp_enable(priv);
	अगर (err)
		जाओ err_destroy_tables;

	वापस 0;

err_destroy_tables:
	जबतक (--i >= 0)
		accel_fs_tcp_destroy_table(priv, i);

	kमुक्त(priv->fs.accel_tcp);
	priv->fs.accel_tcp = शून्य;
	वापस err;
पूर्ण
