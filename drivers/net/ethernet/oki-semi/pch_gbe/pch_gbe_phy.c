<शैली गुरु>
// SPDX-License-Identअगरier: GPL-2.0-only
/*
 * Copyright (C) 1999 - 2010 Intel Corporation.
 * Copyright (C) 2010 OKI SEMICONDUCTOR Co., LTD.
 *
 * This code was derived from the Intel e1000e Linux driver.
 */

#समावेश "pch_gbe.h"
#समावेश "pch_gbe_phy.h"

#घोषणा PHY_MAX_REG_ADDRESS   0x1F	/* 5 bit address bus (0-0x1F) */

/* PHY 1000 MII Register/Bit Definitions */
/* PHY Registers defined by IEEE */
#घोषणा PHY_CONTROL           0x00  /* Control Register */
#घोषणा PHY_STATUS            0x01  /* Status Regiser */
#घोषणा PHY_ID1               0x02  /* Phy Id Register (word 1) */
#घोषणा PHY_ID2               0x03  /* Phy Id Register (word 2) */
#घोषणा PHY_AUTONEG_ADV       0x04  /* Autoneg Advertisement */
#घोषणा PHY_LP_ABILITY        0x05  /* Link Partner Ability (Base Page) */
#घोषणा PHY_AUTONEG_EXP       0x06  /* Autoneg Expansion Register */
#घोषणा PHY_NEXT_PAGE_TX      0x07  /* Next Page TX */
#घोषणा PHY_LP_NEXT_PAGE      0x08  /* Link Partner Next Page */
#घोषणा PHY_1000T_CTRL        0x09  /* 1000Base-T Control Register */
#घोषणा PHY_1000T_STATUS      0x0A  /* 1000Base-T Status Register */
#घोषणा PHY_EXT_STATUS        0x0F  /* Extended Status Register */
#घोषणा PHY_PHYSP_CONTROL     0x10  /* PHY Specअगरic Control Register */
#घोषणा PHY_EXT_PHYSP_CONTROL 0x14  /* Extended PHY Specअगरic Control Register */
#घोषणा PHY_LED_CONTROL       0x18  /* LED Control Register */
#घोषणा PHY_EXT_PHYSP_STATUS  0x1B  /* Extended PHY Specअगरic Status Register */

/* PHY Control Register */
#घोषणा MII_CR_SPEED_SELECT_MSB 0x0040	/* bits 6,13: 10=1000, 01=100, 00=10 */
#घोषणा MII_CR_COLL_TEST_ENABLE 0x0080	/* Collision test enable */
#घोषणा MII_CR_FULL_DUPLEX      0x0100	/* FDX =1, half duplex =0 */
#घोषणा MII_CR_RESTART_AUTO_NEG 0x0200	/* Restart स्वतः negotiation */
#घोषणा MII_CR_ISOLATE          0x0400	/* Isolate PHY from MII */
#घोषणा MII_CR_POWER_DOWN       0x0800	/* Power करोwn */
#घोषणा MII_CR_AUTO_NEG_EN      0x1000	/* Auto Neg Enable */
#घोषणा MII_CR_SPEED_SELECT_LSB 0x2000	/* bits 6,13: 10=1000, 01=100, 00=10 */
#घोषणा MII_CR_LOOPBACK         0x4000	/* 0 = normal, 1 = loopback */
#घोषणा MII_CR_RESET            0x8000	/* 0 = normal, 1 = PHY reset */
#घोषणा MII_CR_SPEED_1000       0x0040
#घोषणा MII_CR_SPEED_100        0x2000
#घोषणा MII_CR_SPEED_10         0x0000

/* PHY Status Register */
#घोषणा MII_SR_EXTENDED_CAPS     0x0001	/* Extended रेजिस्टर capabilities */
#घोषणा MII_SR_JABBER_DETECT     0x0002	/* Jabber Detected */
#घोषणा MII_SR_LINK_STATUS       0x0004	/* Link Status 1 = link */
#घोषणा MII_SR_AUTONEG_CAPS      0x0008	/* Auto Neg Capable */
#घोषणा MII_SR_REMOTE_FAULT      0x0010	/* Remote Fault Detect */
#घोषणा MII_SR_AUTONEG_COMPLETE  0x0020	/* Auto Neg Complete */
#घोषणा MII_SR_PREAMBLE_SUPPRESS 0x0040	/* Preamble may be suppressed */
#घोषणा MII_SR_EXTENDED_STATUS   0x0100	/* Ext. status info in Reg 0x0F */
#घोषणा MII_SR_100T2_HD_CAPS     0x0200	/* 100T2 Half Duplex Capable */
#घोषणा MII_SR_100T2_FD_CAPS     0x0400	/* 100T2 Full Duplex Capable */
#घोषणा MII_SR_10T_HD_CAPS       0x0800	/* 10T   Half Duplex Capable */
#घोषणा MII_SR_10T_FD_CAPS       0x1000	/* 10T   Full Duplex Capable */
#घोषणा MII_SR_100X_HD_CAPS      0x2000	/* 100X  Half Duplex Capable */
#घोषणा MII_SR_100X_FD_CAPS      0x4000	/* 100X  Full Duplex Capable */
#घोषणा MII_SR_100T4_CAPS        0x8000	/* 100T4 Capable */

/* AR8031 PHY Debug Registers */
#घोषणा PHY_AR803X_ID           0x00001374
#घोषणा PHY_AR8031_DBG_OFF      0x1D
#घोषणा PHY_AR8031_DBG_DAT      0x1E
#घोषणा PHY_AR8031_SERDES       0x05
#घोषणा PHY_AR8031_HIBERNATE    0x0B
#घोषणा PHY_AR8031_SERDES_TX_CLK_DLY   0x0100 /* TX घड़ी delay of 2.0ns */
#घोषणा PHY_AR8031_PS_HIB_EN           0x8000 /* Hibernate enable */

/* Phy Id Register (word 2) */
#घोषणा PHY_REVISION_MASK        0x000F

/* PHY Specअगरic Control Register */
#घोषणा PHYSP_CTRL_ASSERT_CRS_TX  0x0800


/* Default value of PHY रेजिस्टर */
#घोषणा PHY_CONTROL_DEFAULT         0x1140 /* Control Register */
#घोषणा PHY_AUTONEG_ADV_DEFAULT     0x01e0 /* Autoneg Advertisement */
#घोषणा PHY_NEXT_PAGE_TX_DEFAULT    0x2001 /* Next Page TX */
#घोषणा PHY_1000T_CTRL_DEFAULT      0x0300 /* 1000Base-T Control Register */
#घोषणा PHY_PHYSP_CONTROL_DEFAULT   0x01EE /* PHY Specअगरic Control Register */

/**
 * pch_gbe_phy_get_id - Retrieve the PHY ID and revision
 * @hw:	       Poपूर्णांकer to the HW काष्ठाure
 * Returns
 *	0:			Successful.
 *	Negative value:		Failed.
 */
s32 pch_gbe_phy_get_id(काष्ठा pch_gbe_hw *hw)
अणु
	काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);
	काष्ठा pch_gbe_phy_info *phy = &hw->phy;
	s32 ret;
	u16 phy_id1;
	u16 phy_id2;

	ret = pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_ID1, &phy_id1);
	अगर (ret)
		वापस ret;
	ret = pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_ID2, &phy_id2);
	अगर (ret)
		वापस ret;
	/*
	 * PHY_ID1: [bit15-0:ID(21-6)]
	 * PHY_ID2: [bit15-10:ID(5-0)][bit9-4:Model][bit3-0:revision]
	 */
	phy->id = (u32)phy_id1;
	phy->id = ((phy->id << 6) | ((phy_id2 & 0xFC00) >> 10));
	phy->revision = (u32) (phy_id2 & 0x000F);
	netdev_dbg(adapter->netdev,
		   "phy->id : 0x%08x  phy->revision : 0x%08x\n",
		   phy->id, phy->revision);
	वापस 0;
पूर्ण

/**
 * pch_gbe_phy_पढ़ो_reg_miic - Read MII control रेजिस्टर
 * @hw:	     Poपूर्णांकer to the HW काष्ठाure
 * @offset:  Register offset to be पढ़ो
 * @data:    Poपूर्णांकer to the पढ़ो data
 * Returns
 *	0:		Successful.
 *	-EINVAL:	Invalid argument.
 */
s32 pch_gbe_phy_पढ़ो_reg_miic(काष्ठा pch_gbe_hw *hw, u32 offset, u16 *data)
अणु
	काष्ठा pch_gbe_phy_info *phy = &hw->phy;

	अगर (offset > PHY_MAX_REG_ADDRESS) अणु
		काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);

		netdev_err(adapter->netdev, "PHY Address %d is out of range\n",
			   offset);
		वापस -EINVAL;
	पूर्ण
	*data = pch_gbe_mac_ctrl_miim(hw, phy->addr, PCH_GBE_HAL_MIIM_READ,
				      offset, (u16)0);
	वापस 0;
पूर्ण

/**
 * pch_gbe_phy_ग_लिखो_reg_miic - Write MII control रेजिस्टर
 * @hw:	     Poपूर्णांकer to the HW काष्ठाure
 * @offset:  Register offset to be पढ़ो
 * @data:    data to ग_लिखो to रेजिस्टर at offset
 * Returns
 *	0:		Successful.
 *	-EINVAL:	Invalid argument.
 */
s32 pch_gbe_phy_ग_लिखो_reg_miic(काष्ठा pch_gbe_hw *hw, u32 offset, u16 data)
अणु
	काष्ठा pch_gbe_phy_info *phy = &hw->phy;

	अगर (offset > PHY_MAX_REG_ADDRESS) अणु
		काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);

		netdev_err(adapter->netdev, "PHY Address %d is out of range\n",
			   offset);
		वापस -EINVAL;
	पूर्ण
	pch_gbe_mac_ctrl_miim(hw, phy->addr, PCH_GBE_HAL_MIIM_WRITE,
				 offset, data);
	वापस 0;
पूर्ण

/**
 * pch_gbe_phy_sw_reset - PHY software reset
 * @hw:	            Poपूर्णांकer to the HW काष्ठाure
 */
अटल व्योम pch_gbe_phy_sw_reset(काष्ठा pch_gbe_hw *hw)
अणु
	u16 phy_ctrl;

	pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_CONTROL, &phy_ctrl);
	phy_ctrl |= MII_CR_RESET;
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_CONTROL, phy_ctrl);
	udelay(1);
पूर्ण

/**
 * pch_gbe_phy_hw_reset - PHY hardware reset
 * @hw:	   Poपूर्णांकer to the HW काष्ठाure
 */
व्योम pch_gbe_phy_hw_reset(काष्ठा pch_gbe_hw *hw)
अणु
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_CONTROL, PHY_CONTROL_DEFAULT);
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_AUTONEG_ADV,
					PHY_AUTONEG_ADV_DEFAULT);
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_NEXT_PAGE_TX,
					PHY_NEXT_PAGE_TX_DEFAULT);
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_1000T_CTRL, PHY_1000T_CTRL_DEFAULT);
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_PHYSP_CONTROL,
					PHY_PHYSP_CONTROL_DEFAULT);
पूर्ण

/**
 * pch_gbe_phy_घातer_up - restore link in हाल the phy was घातered करोwn
 * @hw:	   Poपूर्णांकer to the HW काष्ठाure
 */
व्योम pch_gbe_phy_घातer_up(काष्ठा pch_gbe_hw *hw)
अणु
	u16 mii_reg;

	mii_reg = 0;
	/* Just clear the घातer करोwn bit to wake the phy back up */
	/* according to the manual, the phy will retain its
	 * settings across a घातer-करोwn/up cycle */
	pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_CONTROL, &mii_reg);
	mii_reg &= ~MII_CR_POWER_DOWN;
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_CONTROL, mii_reg);
पूर्ण

/**
 * pch_gbe_phy_घातer_करोwn - Power करोwn PHY
 * @hw:	   Poपूर्णांकer to the HW काष्ठाure
 */
व्योम pch_gbe_phy_घातer_करोwn(काष्ठा pch_gbe_hw *hw)
अणु
	u16 mii_reg;

	mii_reg = 0;
	/* Power करोwn the PHY so no link is implied when पूर्णांकerface is करोwn *
	 * The PHY cannot be घातered करोwn अगर any of the following is TRUE *
	 * (a) WoL is enabled
	 * (b) AMT is active
	 */
	pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_CONTROL, &mii_reg);
	mii_reg |= MII_CR_POWER_DOWN;
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_CONTROL, mii_reg);
	mdelay(1);
पूर्ण

/**
 * pch_gbe_phy_set_rgmii - RGMII पूर्णांकerface setting
 * @hw:	            Poपूर्णांकer to the HW काष्ठाure
 */
व्योम pch_gbe_phy_set_rgmii(काष्ठा pch_gbe_hw *hw)
अणु
	pch_gbe_phy_sw_reset(hw);
पूर्ण

/**
 * pch_gbe_phy_tx_clk_delay - Setup TX घड़ी delay via the PHY
 * @hw:	            Poपूर्णांकer to the HW काष्ठाure
 * Returns
 *	0:		Successful.
 *	-EINVAL:	Invalid argument.
 */
अटल पूर्णांक pch_gbe_phy_tx_clk_delay(काष्ठा pch_gbe_hw *hw)
अणु
	/* The RGMII पूर्णांकerface requires a ~2ns TX घड़ी delay. This is typically
	 * करोne in layout with a दीर्घer trace or via PHY strapping, but can also
	 * be करोne via PHY configuration रेजिस्टरs.
	 */
	काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);
	u16 mii_reg;
	पूर्णांक ret = 0;

	चयन (hw->phy.id) अणु
	हाल PHY_AR803X_ID:
		netdev_dbg(adapter->netdev,
			   "Configuring AR803X PHY for 2ns TX clock delay\n");
		pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_AR8031_DBG_OFF, &mii_reg);
		ret = pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_AR8031_DBG_OFF,
						 PHY_AR8031_SERDES);
		अगर (ret)
			अवरोध;

		pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_AR8031_DBG_DAT, &mii_reg);
		mii_reg |= PHY_AR8031_SERDES_TX_CLK_DLY;
		ret = pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_AR8031_DBG_DAT,
						 mii_reg);
		अवरोध;
	शेष:
		netdev_err(adapter->netdev,
			   "Unknown PHY (%x), could not set TX clock delay\n",
			   hw->phy.id);
		वापस -EINVAL;
	पूर्ण

	अगर (ret)
		netdev_err(adapter->netdev,
			   "Could not configure tx clock delay for PHY\n");
	वापस ret;
पूर्ण

/**
 * pch_gbe_phy_init_setting - PHY initial setting
 * @hw:	            Poपूर्णांकer to the HW काष्ठाure
 */
व्योम pch_gbe_phy_init_setting(काष्ठा pch_gbe_hw *hw)
अणु
	काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);
	काष्ठा ethtool_cmd     cmd = अणु .cmd = ETHTOOL_GSET पूर्ण;
	पूर्णांक ret;
	u16 mii_reg;

	ret = mii_ethtool_gset(&adapter->mii, &cmd);
	अगर (ret)
		netdev_err(adapter->netdev, "Error: mii_ethtool_gset\n");

	ethtool_cmd_speed_set(&cmd, hw->mac.link_speed);
	cmd.duplex = hw->mac.link_duplex;
	cmd.advertising = hw->phy.स्वतःneg_advertised;
	cmd.स्वतःneg = hw->mac.स्वतःneg;
	pch_gbe_phy_ग_लिखो_reg_miic(hw, MII_BMCR, BMCR_RESET);
	ret = mii_ethtool_sset(&adapter->mii, &cmd);
	अगर (ret)
		netdev_err(adapter->netdev, "Error: mii_ethtool_sset\n");

	pch_gbe_phy_sw_reset(hw);

	pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_PHYSP_CONTROL, &mii_reg);
	mii_reg |= PHYSP_CTRL_ASSERT_CRS_TX;
	pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_PHYSP_CONTROL, mii_reg);

	/* Setup a TX घड़ी delay on certain platक्रमms */
	अगर (adapter->pdata && adapter->pdata->phy_tx_clk_delay)
		pch_gbe_phy_tx_clk_delay(hw);
पूर्ण

/**
 * pch_gbe_phy_disable_hibernate - Disable the PHY low घातer state
 * @hw:	            Poपूर्णांकer to the HW काष्ठाure
 * Returns
 *	0:		Successful.
 *	-EINVAL:	Invalid argument.
 */
पूर्णांक pch_gbe_phy_disable_hibernate(काष्ठा pch_gbe_hw *hw)
अणु
	काष्ठा pch_gbe_adapter *adapter = pch_gbe_hw_to_adapter(hw);
	u16 mii_reg;
	पूर्णांक ret = 0;

	चयन (hw->phy.id) अणु
	हाल PHY_AR803X_ID:
		netdev_dbg(adapter->netdev,
			   "Disabling hibernation for AR803X PHY\n");
		ret = pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_AR8031_DBG_OFF,
						 PHY_AR8031_HIBERNATE);
		अगर (ret)
			अवरोध;

		pch_gbe_phy_पढ़ो_reg_miic(hw, PHY_AR8031_DBG_DAT, &mii_reg);
		mii_reg &= ~PHY_AR8031_PS_HIB_EN;
		ret = pch_gbe_phy_ग_लिखो_reg_miic(hw, PHY_AR8031_DBG_DAT,
						 mii_reg);
		अवरोध;
	शेष:
		netdev_err(adapter->netdev,
			   "Unknown PHY (%x), could not disable hibernation\n",
			   hw->phy.id);
		वापस -EINVAL;
	पूर्ण

	अगर (ret)
		netdev_err(adapter->netdev,
			   "Could not disable PHY hibernation\n");
	वापस ret;
पूर्ण
