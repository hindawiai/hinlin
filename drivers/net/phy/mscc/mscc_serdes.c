<शैली गुरु>
// SPDX-License-Identअगरier: (GPL-2.0 OR MIT)
/*
 * Driver क्रम Microsemi VSC85xx PHYs
 *
 * Author: Bjarni Jonasson <bjarni.jonassoni@microchip.com>
 * License: Dual MIT/GPL
 * Copyright (c) 2021 Microsemi Corporation
 */

#समावेश <linux/phy.h>
#समावेश "mscc_serdes.h"
#समावेश "mscc.h"

अटल पूर्णांक pll5g_detune(काष्ठा phy_device *phydev)
अणु
	u32 rd_dat;
	पूर्णांक ret;

	rd_dat = vsc85xx_csr_पढ़ो(phydev, MACRO_CTRL, PHY_S6G_PLL5G_CFG2);
	rd_dat &= ~PHY_S6G_PLL5G_CFG2_GAIN_MASK;
	rd_dat |= PHY_S6G_PLL5G_CFG2_ENA_GAIN;
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_PLL5G_CFG2, rd_dat);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक pll5g_tune(काष्ठा phy_device *phydev)
अणु
	u32 rd_dat;
	पूर्णांक ret;

	rd_dat = vsc85xx_csr_पढ़ो(phydev, MACRO_CTRL, PHY_S6G_PLL5G_CFG2);
	rd_dat &= ~PHY_S6G_PLL5G_CFG2_ENA_GAIN;
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_PLL5G_CFG2, rd_dat);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_pll_cfg_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 pll_ena_offs,
				   स्थिर u32 pll_fsm_ctrl_data,
				   स्थिर u32 pll_fsm_ena)
अणु
	पूर्णांक ret;

	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_PLL_CFG,
				(pll_fsm_ena << PHY_S6G_PLL_ENA_OFFS_POS) |
				(pll_fsm_ctrl_data << PHY_S6G_PLL_FSM_CTRL_DATA_POS) |
				(pll_ena_offs << PHY_S6G_PLL_FSM_ENA_POS));
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_common_cfg_wr(काष्ठा phy_device *phydev,
				      स्थिर u32 sys_rst,
				      स्थिर u32 ena_lane,
				      स्थिर u32 ena_loop,
				      स्थिर u32 qrate,
				      स्थिर u32 अगर_mode,
				      स्थिर u32 pwd_tx)
अणु
	/* ena_loop = 8 क्रम eloop */
	/*          = 4 क्रम floop */
	/*          = 2 क्रम iloop */
	/*          = 1 क्रम ploop */
	/* qrate    = 1 क्रम SGMII, 0 क्रम QSGMII */
	/* अगर_mode  = 1 क्रम SGMII, 3 क्रम QSGMII */

	पूर्णांक ret;

	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_COMMON_CFG,
				(sys_rst << PHY_S6G_SYS_RST_POS) |
				(ena_lane << PHY_S6G_ENA_LANE_POS) |
				(ena_loop << PHY_S6G_ENA_LOOP_POS) |
				(qrate << PHY_S6G_QRATE_POS) |
				(अगर_mode << PHY_S6G_IF_MODE_POS));
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_des_cfg_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 des_phy_ctrl,
				   स्थिर u32 des_mbtr_ctrl,
				   स्थिर u32 des_bw_hyst,
				   स्थिर u32 des_bw_ana,
				   स्थिर u32 des_cpmd_sel)
अणु
	u32 reg_val;
	पूर्णांक ret;

	/* configurable terms */
	reg_val = (des_phy_ctrl << PHY_S6G_DES_PHY_CTRL_POS) |
		  (des_mbtr_ctrl << PHY_S6G_DES_MBTR_CTRL_POS) |
		  (des_cpmd_sel << PHY_S6G_DES_CPMD_SEL_POS) |
		  (des_bw_hyst << PHY_S6G_DES_BW_HYST_POS) |
		  (des_bw_ana << PHY_S6G_DES_BW_ANA_POS);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_DES_CFG,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_ib_cfg0_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 ib_rtrm_adj,
				   स्थिर u32 ib_sig_det_clk_sel,
				   स्थिर u32 ib_reg_pat_sel_offset,
				   स्थिर u32 ib_cal_ena)
अणु
	u32 base_val;
	u32 reg_val;
	पूर्णांक ret;

	/* स्थिरant terms */
	base_val = 0x60a85837;
	/* configurable terms */
	reg_val = base_val | (ib_rtrm_adj << 25) |
		  (ib_sig_det_clk_sel << 16) |
		  (ib_reg_pat_sel_offset << 8) |
		  (ib_cal_ena << 3);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_CFG0,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_ib_cfg1_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 ib_tjtag,
				   स्थिर u32 ib_tsdet,
				   स्थिर u32 ib_scaly,
				   स्थिर u32 ib_frc_offset,
				   स्थिर u32 ib_filt_offset)
अणु
	u32 ib_filt_val;
	u32 reg_val = 0;
	पूर्णांक ret;

	/* स्थिरant terms */
	ib_filt_val = 0xe0;
	/* configurable terms */
	reg_val  = (ib_tjtag << 17) + (ib_tsdet << 12) + (ib_scaly << 8) +
		   ib_filt_val + (ib_filt_offset << 4) + (ib_frc_offset << 0);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_CFG1,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_ib_cfg2_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 ib_tinfv,
				   स्थिर u32 ib_tcalv,
				   स्थिर u32 ib_ureg)
अणु
	u32 ib_cfg2_val;
	u32 base_val;
	पूर्णांक ret;

	/* स्थिरant terms */
	base_val = 0x0f878010;
	/* configurable terms */
	ib_cfg2_val = base_val | ((ib_tinfv) << 28) | ((ib_tcalv) << 5) |
		      (ib_ureg << 0);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_CFG2,
				ib_cfg2_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_ib_cfg3_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 ib_ini_hp,
				   स्थिर u32 ib_ini_mid,
				   स्थिर u32 ib_ini_lp,
				   स्थिर u32 ib_ini_offset)
अणु
	u32 reg_val;
	पूर्णांक ret;

	reg_val  = (ib_ini_hp << 24) + (ib_ini_mid << 16) +
		   (ib_ini_lp << 8) + (ib_ini_offset << 0);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_CFG3,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_ib_cfg4_wr(काष्ठा phy_device *phydev,
				   स्थिर u32 ib_max_hp,
				   स्थिर u32 ib_max_mid,
				   स्थिर u32 ib_max_lp,
				   स्थिर u32 ib_max_offset)
अणु
	u32 reg_val;
	पूर्णांक ret;

	reg_val  = (ib_max_hp << 24) + (ib_max_mid << 16) +
		   (ib_max_lp << 8) + (ib_max_offset << 0);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_CFG4,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_misc_cfg_wr(काष्ठा phy_device *phydev,
				    स्थिर u32 lane_rst)
अणु
	पूर्णांक ret;

	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_MISC_CFG,
				lane_rst);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_gp_cfg_wr(काष्ठा phy_device *phydev, स्थिर u32 gp_cfg_val)
अणु
	पूर्णांक ret;

	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_GP_CFG,
				gp_cfg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_dft_cfg2_wr(काष्ठा phy_device *phydev,
				    स्थिर u32 rx_ji_ampl,
				    स्थिर u32 rx_step_freq,
				    स्थिर u32 rx_ji_ena,
				    स्थिर u32 rx_waveक्रमm_sel,
				    स्थिर u32 rx_freqoff_dir,
				    स्थिर u32 rx_freqoff_ena)
अणु
	u32 reg_val;
	पूर्णांक ret;

	/* configurable terms */
	reg_val = (rx_ji_ampl << 8) | (rx_step_freq << 4) |
		  (rx_ji_ena << 3) | (rx_waveक्रमm_sel << 2) |
		  (rx_freqoff_dir << 1) | rx_freqoff_ena;
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_IB_DFT_CFG2,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

अटल पूर्णांक vsc85xx_sd6g_dft_cfg0_wr(काष्ठा phy_device *phydev,
				    स्थिर u32 prbs_sel,
				    स्थिर u32 test_mode,
				    स्थिर u32 rx_dft_ena)
अणु
	u32 reg_val;
	पूर्णांक ret;

	/* configurable terms */
	reg_val = (prbs_sel << 20) | (test_mode << 16) | (rx_dft_ena << 2);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_DFT_CFG0,
				reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

/* Access LCPLL Cfg_0 */
अटल पूर्णांक vsc85xx_pll5g_cfg0_wr(काष्ठा phy_device *phydev,
				 स्थिर u32 selbgv820)
अणु
	u32 base_val;
	u32 reg_val;
	पूर्णांक ret;

	/* स्थिरant terms */
	base_val = 0x7036f145;
	/* configurable terms */
	reg_val = base_val | (selbgv820 << 23);
	ret = vsc85xx_csr_ग_लिखो(phydev, MACRO_CTRL,
				PHY_S6G_PLL5G_CFG0, reg_val);
	अगर (ret)
		dev_err(&phydev->mdio.dev, "%s: write error\n", __func__);
	वापस ret;
पूर्ण

पूर्णांक vsc85xx_sd6g_config_v2(काष्ठा phy_device *phydev)
अणु
	u32 ib_sig_det_clk_sel_cal = 0;
	u32 ib_sig_det_clk_sel_mm  = 7;
	u32 pll_fsm_ctrl_data = 60;
	अचिन्हित दीर्घ deadline;
	u32 des_bw_ana_val = 3;
	u32 ib_tsdet_cal = 16;
	u32 ib_tsdet_mm  = 5;
	u32 ib_rtrm_adj;
	u32 अगर_mode = 1;
	u32 gp_iter = 5;
	u32 val32 = 0;
	u32 qrate = 1;
	u32 iter;
	पूर्णांक val = 0;
	पूर्णांक ret;

	phy_base_ग_लिखो(phydev, MSCC_EXT_PAGE_ACCESS, MSCC_PHY_PAGE_STANDARD);

	/* Detune/Unlock LCPLL */
	ret = pll5g_detune(phydev);
	अगर (ret)
		वापस ret;

	/* 0. Reset RCPLL */
	ret = vsc85xx_sd6g_pll_cfg_wr(phydev, 3, pll_fsm_ctrl_data, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 0, 0, 0, qrate, अगर_mode, 0);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_des_cfg_wr(phydev, 6, 2, 5, des_bw_ana_val, 0);
	अगर (ret)
		वापस ret;

	/* 1. Configure sd6g क्रम SGMII prior to sd6g_IB_CAL */
	ib_rtrm_adj = 13;
	ret = vsc85xx_sd6g_ib_cfg0_wr(phydev, ib_rtrm_adj, ib_sig_det_clk_sel_mm, 0, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_mm, 15, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg2_wr(phydev, 3, 13, 5);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg3_wr(phydev,  0, 31, 1, 31);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg4_wr(phydev, 63, 63, 2, 63);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 1, 1, 0, qrate, अगर_mode, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_misc_cfg_wr(phydev, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 2. Start rcpll_fsm */
	ret = vsc85xx_sd6g_pll_cfg_wr(phydev, 3, pll_fsm_ctrl_data, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	deadline = jअगरfies + msecs_to_jअगरfies(PROC_CMD_NCOMPLETED_TIMEOUT_MS);
	करो अणु
		usleep_range(500, 1000);
		ret = phy_update_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
		अगर (ret)
			वापस ret;
		val32 = vsc85xx_csr_पढ़ो(phydev, MACRO_CTRL,
					 PHY_S6G_PLL_STATUS);
		/* रुको क्रम bit 12 to clear */
	पूर्ण जबतक (समय_beक्रमe(jअगरfies, deadline) && (val32 & BIT(12)));

	अगर (val32 & BIT(12))
		वापस -ETIMEDOUT;

	/* 4. Release digital reset and disable transmitter */
	ret = vsc85xx_sd6g_misc_cfg_wr(phydev, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 1, 1, 0, qrate, अगर_mode, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 5. Apply a frequency offset on RX-side (using पूर्णांकernal FoJi logic) */
	ret = vsc85xx_sd6g_gp_cfg_wr(phydev, 768);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_dft_cfg2_wr(phydev, 0, 2, 0, 0, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_dft_cfg0_wr(phydev, 0, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_des_cfg_wr(phydev, 6, 2, 5, des_bw_ana_val, 2);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 6. Prepare required settings क्रम IBCAL */
	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_cal, 15, 1, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg0_wr(phydev, ib_rtrm_adj, ib_sig_det_clk_sel_cal, 0, 0);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 7. Start IB_CAL */
	ret = vsc85xx_sd6g_ib_cfg0_wr(phydev, ib_rtrm_adj,
				      ib_sig_det_clk_sel_cal, 0, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;
	/* 11 cycles (क्रम ViperA) or 5 cycles (क्रम ViperB & Elise) w/ SW घड़ी */
	क्रम (iter = 0; iter < gp_iter; iter++) अणु
		/* set gp(0) */
		ret = vsc85xx_sd6g_gp_cfg_wr(phydev, 769);
		अगर (ret)
			वापस ret;
		ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
		अगर (ret)
			वापस ret;
		/* clear gp(0) */
		ret = vsc85xx_sd6g_gp_cfg_wr(phydev, 768);
		अगर (ret)
			वापस ret;
		ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
		अगर (ret)
			वापस ret;
	पूर्ण

	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_cal, 15, 1, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_cal, 15, 0, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 8. Wait क्रम IB cal to complete */
	deadline = jअगरfies + msecs_to_jअगरfies(PROC_CMD_NCOMPLETED_TIMEOUT_MS);
	करो अणु
		usleep_range(500, 1000);
		ret = phy_update_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
		अगर (ret)
			वापस ret;
		val32 = vsc85xx_csr_पढ़ो(phydev, MACRO_CTRL,
					 PHY_S6G_IB_STATUS0);
		/* रुको क्रम bit 8 to set */
	पूर्ण जबतक (समय_beक्रमe(jअगरfies, deadline) && (~val32 & BIT(8)));

	अगर (~val32 & BIT(8))
		वापस -ETIMEDOUT;

	/* 9. Restore cfg values क्रम mission mode */
	ret = vsc85xx_sd6g_ib_cfg0_wr(phydev, ib_rtrm_adj, ib_sig_det_clk_sel_mm, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_mm, 15, 0, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 10. Re-enable transmitter */
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 1, 1, 0, qrate, अगर_mode, 0);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 11. Disable frequency offset generation (using पूर्णांकernal FoJi logic) */
	ret = vsc85xx_sd6g_dft_cfg2_wr(phydev, 0, 0, 0, 0, 0, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_dft_cfg0_wr(phydev, 0, 0, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_des_cfg_wr(phydev, 6, 2, 5, des_bw_ana_val, 0);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* Tune/Re-lock LCPLL */
	ret = pll5g_tune(phydev);
	अगर (ret)
		वापस ret;

	/* 12. Configure क्रम Final Configuration and Settings */
	/* a. Reset RCPLL */
	ret = vsc85xx_sd6g_pll_cfg_wr(phydev, 3, pll_fsm_ctrl_data, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 0, 1, 0, qrate, अगर_mode, 0);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* b. Configure sd6g क्रम desired operating mode */
	phy_base_ग_लिखो(phydev, MSCC_EXT_PAGE_ACCESS, MSCC_PHY_PAGE_EXTENDED_GPIO);
	ret = phy_base_पढ़ो(phydev, MSCC_PHY_MAC_CFG_FASTLINK);
	अगर ((ret & MAC_CFG_MASK) == MAC_CFG_QSGMII) अणु
		/* QSGMII */
		pll_fsm_ctrl_data = 120;
		qrate   = 0;
		अगर_mode = 3;
		des_bw_ana_val = 5;
		val = PROC_CMD_MCB_ACCESS_MAC_CONF | PROC_CMD_RST_CONF_PORT |
			PROC_CMD_READ_MOD_WRITE_PORT | PROC_CMD_QSGMII_MAC;

		ret = vsc8584_cmd(phydev, val);
		अगर (ret) अणु
			dev_err(&phydev->mdio.dev, "%s: QSGMII error: %d\n",
				__func__, ret);
			वापस ret;
		पूर्ण

		phy_base_ग_लिखो(phydev, MSCC_EXT_PAGE_ACCESS, MSCC_PHY_PAGE_STANDARD);
	पूर्ण अन्यथा अगर ((ret & MAC_CFG_MASK) == MAC_CFG_SGMII) अणु
		/* SGMII */
		pll_fsm_ctrl_data = 60;
		qrate   = 1;
		अगर_mode = 1;
		des_bw_ana_val = 3;

		val = PROC_CMD_MCB_ACCESS_MAC_CONF | PROC_CMD_RST_CONF_PORT |
			PROC_CMD_READ_MOD_WRITE_PORT | PROC_CMD_SGMII_MAC;

		ret = vsc8584_cmd(phydev, val);
		अगर (ret) अणु
			dev_err(&phydev->mdio.dev, "%s: SGMII error: %d\n",
				__func__, ret);
			वापस ret;
		पूर्ण

		phy_base_ग_लिखो(phydev, MSCC_EXT_PAGE_ACCESS, MSCC_PHY_PAGE_STANDARD);
	पूर्ण अन्यथा अणु
		dev_err(&phydev->mdio.dev, "%s: invalid mac_if: %x\n",
			__func__, ret);
	पूर्ण

	ret = phy_update_mcb_s6g(phydev, PHY_S6G_LCPLL_CFG, 0);
	अगर (ret)
		वापस ret;
	ret = phy_update_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_pll5g_cfg0_wr(phydev, 4);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_S6G_LCPLL_CFG, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_des_cfg_wr(phydev, 6, 2, 5, des_bw_ana_val, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg0_wr(phydev, ib_rtrm_adj, ib_sig_det_clk_sel_mm, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg1_wr(phydev, 8, ib_tsdet_mm, 15, 0, 1);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_common_cfg_wr(phydev, 1, 1, 0, qrate, अगर_mode, 0);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg2_wr(phydev, 3, 13, 5);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg3_wr(phydev,  0, 31, 1, 31);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_ib_cfg4_wr(phydev, 63, 63, 2, 63);
	अगर (ret)
		वापस ret;
	ret = vsc85xx_sd6g_misc_cfg_wr(phydev, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 13. Start rcpll_fsm */
	ret = vsc85xx_sd6g_pll_cfg_wr(phydev, 3, pll_fsm_ctrl_data, 1);
	अगर (ret)
		वापस ret;
	ret = phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
	अगर (ret)
		वापस ret;

	/* 14. Wait क्रम PLL cal to complete */
	deadline = jअगरfies + msecs_to_jअगरfies(PROC_CMD_NCOMPLETED_TIMEOUT_MS);
	करो अणु
		usleep_range(500, 1000);
		ret = phy_update_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
		अगर (ret)
			वापस ret;
		val32 = vsc85xx_csr_पढ़ो(phydev, MACRO_CTRL,
					 PHY_S6G_PLL_STATUS);
		/* रुको क्रम bit 12 to clear */
	पूर्ण जबतक (समय_beक्रमe(jअगरfies, deadline) && (val32 & BIT(12)));

	अगर (val32 & BIT(12))
		वापस -ETIMEDOUT;

	/* release lane reset */
	ret = vsc85xx_sd6g_misc_cfg_wr(phydev, 0);
	अगर (ret)
		वापस ret;

	वापस phy_commit_mcb_s6g(phydev, PHY_MCB_S6G_CFG, 0);
पूर्ण
