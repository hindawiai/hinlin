<शैली गुरु>
/*
 * Copyright 2008 Cisco Systems, Inc.  All rights reserved.
 * Copyright 2007 Nuova Systems, Inc.  All rights reserved.
 *
 * This program is मुक्त software; you may redistribute it and/or modअगरy
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; version 2 of the License.
 *
 * THE SOFTWARE IS PROVIDED "AS IS", WITHOUT WARRANTY OF ANY KIND,
 * EXPRESS OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF
 * MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE AND
 * NONINFRINGEMENT. IN NO EVENT SHALL THE AUTHORS OR COPYRIGHT HOLDERS
 * BE LIABLE FOR ANY CLAIM, DAMAGES OR OTHER LIABILITY, WHETHER IN AN
 * ACTION OF CONTRACT, TORT OR OTHERWISE, ARISING FROM, OUT OF OR IN
 * CONNECTION WITH THE SOFTWARE OR THE USE OR OTHER DEALINGS IN THE
 * SOFTWARE.
 */
#समावेश <linux/त्रुटिसं.स>
#समावेश <linux/types.h>
#समावेश <linux/pci.h>
#समावेश "wq_enet_desc.h"
#समावेश "rq_enet_desc.h"
#समावेश "cq_enet_desc.h"
#समावेश "vnic_resource.h"
#समावेश "vnic_dev.h"
#समावेश "vnic_wq.h"
#समावेश "vnic_rq.h"
#समावेश "vnic_cq.h"
#समावेश "vnic_intr.h"
#समावेश "vnic_stats.h"
#समावेश "vnic_nic.h"
#समावेश "fnic.h"

पूर्णांक fnic_get_vnic_config(काष्ठा fnic *fnic)
अणु
	काष्ठा vnic_fc_config *c = &fnic->config;
	पूर्णांक err;

#घोषणा GET_CONFIG(m) \
	करो अणु \
		err = vnic_dev_spec(fnic->vdev, \
				    दुरत्व(काष्ठा vnic_fc_config, m), \
				    माप(c->m), &c->m); \
		अगर (err) अणु \
			shost_prपूर्णांकk(KERN_ERR, fnic->lport->host, \
				     "Error getting %s, %d\n", #m, \
				     err); \
			वापस err; \
		पूर्ण \
	पूर्ण जबतक (0);

	GET_CONFIG(node_wwn);
	GET_CONFIG(port_wwn);
	GET_CONFIG(wq_enet_desc_count);
	GET_CONFIG(wq_copy_desc_count);
	GET_CONFIG(rq_desc_count);
	GET_CONFIG(maxdatafieldsize);
	GET_CONFIG(ed_tov);
	GET_CONFIG(ra_tov);
	GET_CONFIG(पूर्णांकr_समयr);
	GET_CONFIG(पूर्णांकr_समयr_type);
	GET_CONFIG(flags);
	GET_CONFIG(flogi_retries);
	GET_CONFIG(flogi_समयout);
	GET_CONFIG(plogi_retries);
	GET_CONFIG(plogi_समयout);
	GET_CONFIG(io_throttle_count);
	GET_CONFIG(link_करोwn_समयout);
	GET_CONFIG(port_करोwn_समयout);
	GET_CONFIG(port_करोwn_io_retries);
	GET_CONFIG(luns_per_tgt);

	c->wq_enet_desc_count =
		min_t(u32, VNIC_FNIC_WQ_DESCS_MAX,
		      max_t(u32, VNIC_FNIC_WQ_DESCS_MIN,
			    c->wq_enet_desc_count));
	c->wq_enet_desc_count = ALIGN(c->wq_enet_desc_count, 16);

	c->wq_copy_desc_count =
		min_t(u32, VNIC_FNIC_WQ_COPY_DESCS_MAX,
		      max_t(u32, VNIC_FNIC_WQ_COPY_DESCS_MIN,
			    c->wq_copy_desc_count));
	c->wq_copy_desc_count = ALIGN(c->wq_copy_desc_count, 16);

	c->rq_desc_count =
		min_t(u32, VNIC_FNIC_RQ_DESCS_MAX,
		      max_t(u32, VNIC_FNIC_RQ_DESCS_MIN,
			    c->rq_desc_count));
	c->rq_desc_count = ALIGN(c->rq_desc_count, 16);

	c->maxdatafieldsize =
		min_t(u16, VNIC_FNIC_MAXDATAFIELDSIZE_MAX,
		      max_t(u16, VNIC_FNIC_MAXDATAFIELDSIZE_MIN,
			    c->maxdatafieldsize));
	c->ed_tov =
		min_t(u32, VNIC_FNIC_EDTOV_MAX,
		      max_t(u32, VNIC_FNIC_EDTOV_MIN,
			    c->ed_tov));

	c->ra_tov =
		min_t(u32, VNIC_FNIC_RATOV_MAX,
		      max_t(u32, VNIC_FNIC_RATOV_MIN,
			    c->ra_tov));

	c->flogi_retries =
		min_t(u32, VNIC_FNIC_FLOGI_RETRIES_MAX, c->flogi_retries);

	c->flogi_समयout =
		min_t(u32, VNIC_FNIC_FLOGI_TIMEOUT_MAX,
		      max_t(u32, VNIC_FNIC_FLOGI_TIMEOUT_MIN,
			    c->flogi_समयout));

	c->plogi_retries =
		min_t(u32, VNIC_FNIC_PLOGI_RETRIES_MAX, c->plogi_retries);

	c->plogi_समयout =
		min_t(u32, VNIC_FNIC_PLOGI_TIMEOUT_MAX,
		      max_t(u32, VNIC_FNIC_PLOGI_TIMEOUT_MIN,
			    c->plogi_समयout));

	c->io_throttle_count =
		min_t(u32, VNIC_FNIC_IO_THROTTLE_COUNT_MAX,
		      max_t(u32, VNIC_FNIC_IO_THROTTLE_COUNT_MIN,
			    c->io_throttle_count));

	c->link_करोwn_समयout =
		min_t(u32, VNIC_FNIC_LINK_DOWN_TIMEOUT_MAX,
		      c->link_करोwn_समयout);

	c->port_करोwn_समयout =
		min_t(u32, VNIC_FNIC_PORT_DOWN_TIMEOUT_MAX,
		      c->port_करोwn_समयout);

	c->port_करोwn_io_retries =
		min_t(u32, VNIC_FNIC_PORT_DOWN_IO_RETRIES_MAX,
		      c->port_करोwn_io_retries);

	c->luns_per_tgt =
		min_t(u32, VNIC_FNIC_LUNS_PER_TARGET_MAX,
		      max_t(u32, VNIC_FNIC_LUNS_PER_TARGET_MIN,
			    c->luns_per_tgt));

	c->पूर्णांकr_समयr = min_t(u16, VNIC_INTR_TIMER_MAX, c->पूर्णांकr_समयr);
	c->पूर्णांकr_समयr_type = c->पूर्णांकr_समयr_type;

	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC MAC addr %pM "
		     "wq/wq_copy/rq %d/%d/%d\n",
		     fnic->ctlr.ctl_src_addr,
		     c->wq_enet_desc_count, c->wq_copy_desc_count,
		     c->rq_desc_count);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC node wwn %llx port wwn %llx\n",
		     c->node_wwn, c->port_wwn);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC ed_tov %d ra_tov %d\n",
		     c->ed_tov, c->ra_tov);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC mtu %d intr timer %d\n",
		     c->maxdatafieldsize, c->पूर्णांकr_समयr);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC flags 0x%x luns per tgt %d\n",
		     c->flags, c->luns_per_tgt);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC flogi_retries %d flogi timeout %d\n",
		     c->flogi_retries, c->flogi_समयout);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC plogi retries %d plogi timeout %d\n",
		     c->plogi_retries, c->plogi_समयout);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC io throttle count %d link dn timeout %d\n",
		     c->io_throttle_count, c->link_करोwn_समयout);
	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host,
		     "vNIC port dn io retries %d port dn timeout %d\n",
		     c->port_करोwn_io_retries, c->port_करोwn_समयout);

	वापस 0;
पूर्ण

पूर्णांक fnic_set_nic_config(काष्ठा fnic *fnic, u8 rss_शेष_cpu,
			u8 rss_hash_type,
			u8 rss_hash_bits, u8 rss_base_cpu, u8 rss_enable,
			u8 tso_ipid_split_en, u8 ig_vlan_strip_en)
अणु
	u64 a0, a1;
	u32 nic_cfg;
	पूर्णांक रुको = 1000;

	vnic_set_nic_cfg(&nic_cfg, rss_शेष_cpu,
		rss_hash_type, rss_hash_bits, rss_base_cpu,
		rss_enable, tso_ipid_split_en, ig_vlan_strip_en);

	a0 = nic_cfg;
	a1 = 0;

	वापस vnic_dev_cmd(fnic->vdev, CMD_NIC_CFG, &a0, &a1, रुको);
पूर्ण

व्योम fnic_get_res_counts(काष्ठा fnic *fnic)
अणु
	fnic->wq_count = vnic_dev_get_res_count(fnic->vdev, RES_TYPE_WQ);
	fnic->raw_wq_count = fnic->wq_count - 1;
	fnic->wq_copy_count = fnic->wq_count - fnic->raw_wq_count;
	fnic->rq_count = vnic_dev_get_res_count(fnic->vdev, RES_TYPE_RQ);
	fnic->cq_count = vnic_dev_get_res_count(fnic->vdev, RES_TYPE_CQ);
	fnic->पूर्णांकr_count = vnic_dev_get_res_count(fnic->vdev,
		RES_TYPE_INTR_CTRL);
पूर्ण

व्योम fnic_मुक्त_vnic_resources(काष्ठा fnic *fnic)
अणु
	अचिन्हित पूर्णांक i;

	क्रम (i = 0; i < fnic->raw_wq_count; i++)
		vnic_wq_मुक्त(&fnic->wq[i]);

	क्रम (i = 0; i < fnic->wq_copy_count; i++)
		vnic_wq_copy_मुक्त(&fnic->wq_copy[i]);

	क्रम (i = 0; i < fnic->rq_count; i++)
		vnic_rq_मुक्त(&fnic->rq[i]);

	क्रम (i = 0; i < fnic->cq_count; i++)
		vnic_cq_मुक्त(&fnic->cq[i]);

	क्रम (i = 0; i < fnic->पूर्णांकr_count; i++)
		vnic_पूर्णांकr_मुक्त(&fnic->पूर्णांकr[i]);
पूर्ण

पूर्णांक fnic_alloc_vnic_resources(काष्ठा fnic *fnic)
अणु
	क्रमागत vnic_dev_पूर्णांकr_mode पूर्णांकr_mode;
	अचिन्हित पूर्णांक mask_on_निश्चितion;
	अचिन्हित पूर्णांक पूर्णांकerrupt_offset;
	अचिन्हित पूर्णांक error_पूर्णांकerrupt_enable;
	अचिन्हित पूर्णांक error_पूर्णांकerrupt_offset;
	अचिन्हित पूर्णांक i, cq_index;
	अचिन्हित पूर्णांक wq_copy_cq_desc_count;
	पूर्णांक err;

	पूर्णांकr_mode = vnic_dev_get_पूर्णांकr_mode(fnic->vdev);

	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host, "vNIC interrupt mode: %s\n",
		     पूर्णांकr_mode == VNIC_DEV_INTR_MODE_INTX ? "legacy PCI INTx" :
		     पूर्णांकr_mode == VNIC_DEV_INTR_MODE_MSI ? "MSI" :
		     पूर्णांकr_mode == VNIC_DEV_INTR_MODE_MSIX ?
		     "MSI-X" : "unknown");

	shost_prपूर्णांकk(KERN_INFO, fnic->lport->host, "vNIC resources avail: "
		     "wq %d cp_wq %d raw_wq %d rq %d cq %d intr %d\n",
		     fnic->wq_count, fnic->wq_copy_count, fnic->raw_wq_count,
		     fnic->rq_count, fnic->cq_count, fnic->पूर्णांकr_count);

	/* Allocate Raw WQ used क्रम FCS frames */
	क्रम (i = 0; i < fnic->raw_wq_count; i++) अणु
		err = vnic_wq_alloc(fnic->vdev, &fnic->wq[i], i,
			fnic->config.wq_enet_desc_count,
			माप(काष्ठा wq_enet_desc));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	/* Allocate Copy WQs used क्रम SCSI IOs */
	क्रम (i = 0; i < fnic->wq_copy_count; i++) अणु
		err = vnic_wq_copy_alloc(fnic->vdev, &fnic->wq_copy[i],
			(fnic->raw_wq_count + i),
			fnic->config.wq_copy_desc_count,
			माप(काष्ठा fcpio_host_req));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	/* RQ क्रम receiving FCS frames */
	क्रम (i = 0; i < fnic->rq_count; i++) अणु
		err = vnic_rq_alloc(fnic->vdev, &fnic->rq[i], i,
			fnic->config.rq_desc_count,
			माप(काष्ठा rq_enet_desc));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	/* CQ क्रम each RQ */
	क्रम (i = 0; i < fnic->rq_count; i++) अणु
		cq_index = i;
		err = vnic_cq_alloc(fnic->vdev,
			&fnic->cq[cq_index], cq_index,
			fnic->config.rq_desc_count,
			माप(काष्ठा cq_enet_rq_desc));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	/* CQ क्रम each WQ */
	क्रम (i = 0; i < fnic->raw_wq_count; i++) अणु
		cq_index = fnic->rq_count + i;
		err = vnic_cq_alloc(fnic->vdev, &fnic->cq[cq_index], cq_index,
			fnic->config.wq_enet_desc_count,
			माप(काष्ठा cq_enet_wq_desc));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	/* CQ क्रम each COPY WQ */
	wq_copy_cq_desc_count = (fnic->config.wq_copy_desc_count * 3);
	क्रम (i = 0; i < fnic->wq_copy_count; i++) अणु
		cq_index = fnic->raw_wq_count + fnic->rq_count + i;
		err = vnic_cq_alloc(fnic->vdev, &fnic->cq[cq_index],
			cq_index,
			wq_copy_cq_desc_count,
			माप(काष्ठा fcpio_fw_req));
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	क्रम (i = 0; i < fnic->पूर्णांकr_count; i++) अणु
		err = vnic_पूर्णांकr_alloc(fnic->vdev, &fnic->पूर्णांकr[i], i);
		अगर (err)
			जाओ err_out_cleanup;
	पूर्ण

	fnic->legacy_pba = vnic_dev_get_res(fnic->vdev,
				RES_TYPE_INTR_PBA_LEGACY, 0);

	अगर (!fnic->legacy_pba && पूर्णांकr_mode == VNIC_DEV_INTR_MODE_INTX) अणु
		shost_prपूर्णांकk(KERN_ERR, fnic->lport->host,
			     "Failed to hook legacy pba resource\n");
		err = -ENODEV;
		जाओ err_out_cleanup;
	पूर्ण

	/*
	 * Init RQ/WQ resources.
	 *
	 * RQ[0 to n-1] poपूर्णांक to CQ[0 to n-1]
	 * WQ[0 to m-1] poपूर्णांक to CQ[n to n+m-1]
	 * WQ_COPY[0 to k-1] poपूर्णांकs to CQ[n+m to n+m+k-1]
	 *
	 * Note क्रम copy wq we always initialize with cq_index = 0
	 *
	 * Error पूर्णांकerrupt is not enabled क्रम MSI.
	 */

	चयन (पूर्णांकr_mode) अणु
	हाल VNIC_DEV_INTR_MODE_INTX:
	हाल VNIC_DEV_INTR_MODE_MSIX:
		error_पूर्णांकerrupt_enable = 1;
		error_पूर्णांकerrupt_offset = fnic->err_पूर्णांकr_offset;
		अवरोध;
	शेष:
		error_पूर्णांकerrupt_enable = 0;
		error_पूर्णांकerrupt_offset = 0;
		अवरोध;
	पूर्ण

	क्रम (i = 0; i < fnic->rq_count; i++) अणु
		cq_index = i;
		vnic_rq_init(&fnic->rq[i],
			     cq_index,
			     error_पूर्णांकerrupt_enable,
			     error_पूर्णांकerrupt_offset);
	पूर्ण

	क्रम (i = 0; i < fnic->raw_wq_count; i++) अणु
		cq_index = i + fnic->rq_count;
		vnic_wq_init(&fnic->wq[i],
			     cq_index,
			     error_पूर्णांकerrupt_enable,
			     error_पूर्णांकerrupt_offset);
	पूर्ण

	क्रम (i = 0; i < fnic->wq_copy_count; i++) अणु
		vnic_wq_copy_init(&fnic->wq_copy[i],
				  0 /* cq_index 0 - always */,
				  error_पूर्णांकerrupt_enable,
				  error_पूर्णांकerrupt_offset);
	पूर्ण

	क्रम (i = 0; i < fnic->cq_count; i++) अणु

		चयन (पूर्णांकr_mode) अणु
		हाल VNIC_DEV_INTR_MODE_MSIX:
			पूर्णांकerrupt_offset = i;
			अवरोध;
		शेष:
			पूर्णांकerrupt_offset = 0;
			अवरोध;
		पूर्ण

		vnic_cq_init(&fnic->cq[i],
			0 /* flow_control_enable */,
			1 /* color_enable */,
			0 /* cq_head */,
			0 /* cq_tail */,
			1 /* cq_tail_color */,
			1 /* पूर्णांकerrupt_enable */,
			1 /* cq_entry_enable */,
			0 /* cq_message_enable */,
			पूर्णांकerrupt_offset,
			0 /* cq_message_addr */);
	पूर्ण

	/*
	 * Init INTR resources
	 *
	 * mask_on_निश्चितion is not used क्रम INTx due to the level-
	 * triggered nature of INTx
	 */

	चयन (पूर्णांकr_mode) अणु
	हाल VNIC_DEV_INTR_MODE_MSI:
	हाल VNIC_DEV_INTR_MODE_MSIX:
		mask_on_निश्चितion = 1;
		अवरोध;
	शेष:
		mask_on_निश्चितion = 0;
		अवरोध;
	पूर्ण

	क्रम (i = 0; i < fnic->पूर्णांकr_count; i++) अणु
		vnic_पूर्णांकr_init(&fnic->पूर्णांकr[i],
			fnic->config.पूर्णांकr_समयr,
			fnic->config.पूर्णांकr_समयr_type,
			mask_on_निश्चितion);
	पूर्ण

	/* init the stats memory by making the first call here */
	err = vnic_dev_stats_dump(fnic->vdev, &fnic->stats);
	अगर (err) अणु
		shost_prपूर्णांकk(KERN_ERR, fnic->lport->host,
			     "vnic_dev_stats_dump failed - x%x\n", err);
		जाओ err_out_cleanup;
	पूर्ण

	/* Clear LIF stats */
	vnic_dev_stats_clear(fnic->vdev);

	वापस 0;

err_out_cleanup:
	fnic_मुक्त_vnic_resources(fnic);

	वापस err;
पूर्ण
