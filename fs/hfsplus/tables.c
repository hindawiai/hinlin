...
/*
 *  Unicode हाल folding table taken from Apple Technote #1150
 *  (HFS Plus Volume Format)
 */

u16 hfsplus_हाल_fold_table[] = अणु
/*
 *  The lower हाल table consists of a 256-entry high-byte table followed by
 *  some number of 256-entry subtables. The high-byte table contains either an
 *  offset to the subtable क्रम अक्षरacters with that high byte or zero, which
 *  means that there are no हाल mappings or ignored अक्षरacters in that block.
 *  Ignored अक्षरacters are mapped to zero.
 */

    // High-byte indices ( == 0 अगरf no हाल mapping and no ignorables )
...
u16 hfsplus_decompose_table[] = अणु
	/* base table */
