...
/*
 * usnjrnl.h - Defines क्रम NTFS kernel transaction log ($UsnJrnl) handling.
 *	       Part of the Linux-NTFS project.
 *
 * Copyright (c) 2005 Anton Altaparmakov
 */

#अगर_अघोषित _LINUX_NTFS_USNJRNL_H
#घोषणा _LINUX_NTFS_USNJRNL_H
 ...
 *
 * The transaction log records whenever a file is modअगरied in any way.  So क्रम
 * example it will record that file "blah" was written to at a particular समय
 * but not what was written.  If will record that a file was deleted or
 * created, that a file was truncated, etc.  See below क्रम all the reason
 * codes used.
 *
 ...
 * either enabled or in the process of being disabled in which हाल we can
 * ignore it as it will go away as soon as Winकरोws माला_लो its hands on it.
 *
 ...
 * $VOLUME_INFORMATION attribute in the $Volume प्रणाली file (which is present
 ...
 * is in the process of being disabled and अगर this flag is clear it means the
 ...
 * sized USN_RECORDs (see below क्रम all the काष्ठाures).
 *
 * We करो not care about transaction logging at this poपूर्णांक in समय but we still
 * need to let winकरोws know that the transaction log is out of date.  To करो
 ...
 * क्रम the next added USN_RECORD to the $DATA/$J attribute as well as
 * generating a new journal_id in $DATA/$Max.
 *
 * The journal_id is as of the current version (2.0) of the transaction log
 * simply the 64-bit बारtamp of when the journal was either created or last
 * stamped.
 *
 ...
 * are simply byte offsets पूर्णांकo $DATA/$J, this is exactly the next usn.  For
 ...
 * $STANDARD_INFORMATION attribute to zero.  Once that is करोne, one would need
 ...
 *
 * Note that अगर a volume is unmounted whilst the transaction log is being
 * disabled, the process will जारी the next समय the volume is mounted.
 * This is why we can safely mount पढ़ो-ग_लिखो when we see a transaction log
 * in the process of being deleted.
 */

/* Some $UsnJrnl related स्थिरants. */
#घोषणा UsnJrnlMajorVer		2
#घोषणा UsnJrnlMinorVer		0
...
/*0x18*/leUSN lowest_valid_usn;	/* Lowest valid usn in $DATA/$J क्रम the
				   current journal_id. */
/* माप() = 32 (0x20) bytes */
पूर्ण __attribute__ ((__packed__)) USN_HEADER;

/*
 * Reason flags (32-bit).  Cumulative flags describing the change(s) to the
 * file since it was last खोलोed.  I think the names speak क्रम themselves but
 * अगर you disagree check out the descriptions in the Linux NTFS project NTFS
 * करोcumentation: http://www.linux-ntfs.org/
 */
क्रमागत अणु
	...
	USN_REASON_खाता_CREATE		= cpu_to_le32(0x00000100),
	USN_REASON_खाता_DELETE		= cpu_to_le32(0x00000200),
...
प्रकार le32 USN_REASON_FLAGS;

/*
 * Source info flags (32-bit).  Inक्रमmation about the source of the change(s)
 * to the file.  For detailed descriptions of what these mean, see the Linux
 * NTFS project NTFS करोcumentation:
...
प्रकार le32 USN_SOURCE_INFO_FLAGS;
 ...
 * specअगरied in $DATA/$Max.  When the maximum_size specअगरied in $DATA/$Max is
 ...
 * bytes at the beginning of the attribute are मुक्तd and made sparse.  Note the
 * making sparse only happens at volume checkpoपूर्णांकs and hence the actual
...
/*0x20*/sle64 समय;		/* Time when this record was created. */
...
/*0x34*/खाता_ATTR_FLAGS file_attributes;	/* File attributes copied from
				   $STANDARD_INFORMATION or $खाता_NAME (not
				   sure which). */
...
/*0x3c*/ntfsअक्षर file_name[0];	/* Use when creating only.  When पढ़ोing use
				   file_name_offset to determine the location
				   of the name. */
/* माप() = 60 (0x3c) bytes */
पूर्ण __attribute__ ((__packed__)) USN_RECORD;

बाह्य bool ntfs_stamp_usnjrnl(ntfs_volume *vol);

#पूर्ण_अगर /* NTFS_RW */

#पूर्ण_अगर /* _LINUX_NTFS_USNJRNL_H */
