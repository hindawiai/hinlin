...
|			All Rights Reserved
|
|       For details on the license क्रम this file, please see the
...
|		unlk	a6
|
|	and then either "bra fpsp_done" अगर the exception was completely
|	handled	by the package, or "bra real_xxxx" which is an बाह्यal
|	label to a routine that will process a real exception of the
|	type that was generated.  Some handlers may omit the "frestore"
|	अगर the FPU state after the exception is idle.
|
|	Someबार the exception handler will transक्रमm the fsave area
|	because it needs to report an exception back to the user.  This
|	can happen अगर the package is entered क्रम an unimplemented भग्न
|	inकाष्ठाion that generates (say) an underflow.  Alternatively,
|	a second fsave frame can be pushed onto the stack and the
|	handler	निकास code will reload the new frame and discard the old.
|
|	The रेजिस्टरs d0, d1, a0, a1 and fp0-fp3 are always saved and
|	restored from the "local variable" area and can be used as
|	temporaries.  If a routine needs to change any
|	of these रेजिस्टरs, it should modअगरy the saved copy and let
|	the handler निकास code restore the value.
|
|----------------------------------------------------------------------
|
|	Local Variables on the stack
|
	.set	LOCAL_SIZE,192		| bytes needed क्रम local variables
	.set	LV,-LOCAL_SIZE	| convenient base value
|
	.set	USER_DA,LV+0		| save space क्रम D0-D1,A0-A1
	...
	.set	FP_SCR1,LV+76		| room क्रम a temporary भग्न value
	.set	FP_SCR2,LV+92		| room क्रम a temporary भग्न value
	.set	L_SCR1,LV+108		| room क्रम a temporary दीर्घ value
	.set	L_SCR2,LV+112		| room क्रम a temporary दीर्घ value
	...
	.set	UFLG_TMP,LV+121		| temporary क्रम uflag errata
	.set	CU_ONLY,LV+122		| cu-only flag
	.set	VER_TMP,LV+123		| temp holding क्रम version number
	.set	L_SCR3,LV+124		| room क्रम a temporary दीर्घ value
	.set	FP_SCR3,LV+128		| room क्रम a temporary भग्न value
	.set	FP_SCR4,LV+144		| room क्रम a temporary भग्न value
	.set	FP_SCR5,LV+160		| room क्रम a temporary भग्न value
	.set	FP_SCR6,LV+176
|
...
|
	.set	CU_SAVEPC,LV-92		| micro-pc क्रम CU (1 byte)
	.set	FPR_सूचीTY_BITS,LV-91		| fpr dirty bits
|
	.set	WBTEMP,LV-76		| ग_लिखो back temp (12 bytes)
...
|
	.set	FPSR_SHADOW,LV-64		| fpsr shaकरोw reg
|
	.set	FPIARCU,LV-60		| Instr. addr. reg. क्रम CU (4 bytes)
|
	.set	CMDREG2B,LV-52		| cmd reg क्रम machine 2
	.set	CMDREG3B,LV-48		| cmd reg क्रम E3 exceptions (2 bytes)
|
...
|
	...
	.set	denorm_bit,7		| bit determines अगर denorm or unnorm
...
|
	...
	.set	sticky_bit,7
|
	.set	CMDREG1B,LV-36		| cmd reg क्रम E1 exceptions (2 bytes)
	.set	kfact_bit,12		| distinguishes अटल/dynamic k-factor
...
|
	.set	EXC_SR,4		| exception frame status रेजिस्टर
	.set	EXC_PC,6		| exception frame program counter
	.set	EXC_VEC,10		| exception frame vector (क्रमmat+vector#)
...
|
	...
	.set	snan_bit,6	|  संकेतling nan
	.set	operr_bit,5	|  opeअक्रम error
	.set	ovfl_bit,4	|  overflow
	.set	unfl_bit,3	|  underflow
	.set	dz_bit,2	|  भागide by zero
...
|
	...
	.set	adz_bit,4	|  accrued भागide by zero
	.set	ainex_bit,3	|  accrued inexact
|
|	FPSR inभागidual bit masks
|
...
|
	...
	.set	adz_mask,0x00000010	|  accrued भागide by zero
...
|
	...
	.set	d_mode,0x80	|  round to द्विगुन
|
...
|
	.set	signan_bit,6	|  संकेतling nan bit in mantissa
	.set	sign_bit,7
|
	.set	rnd_stky_bit,29	|  round/sticky bit of mantissa
|				this can only be used अगर in a data रेजिस्टर
...
|
	...
	.set	LOCAL_HI,4
	.set	LOCAL_LO,8
	.set	LOCAL_GRS,12	|  valid ONLY क्रम FP_SCR1, FP_SCR2
|
|
	.set	norm_tag,0x00	|  tag bits in अणु7:5पूर्ण position
	.set	zero_tag,0x20
	.set	inf_tag,0x40
	.set	nan_tag,0x60
	.set	dnrm_tag,0x80
|
|	fsave sizes and क्रमmats
|
	.set	VER_4,0x40		|  fpsp compatible version numbers
|					are in the $40s अणु$40-$4fपूर्ण
