...
/*
 * Bits in event code क्रम POWER5 (not POWER5++)
 */
...
अटल अचिन्हित दीर्घ unit_cons[PM_LASTUNIT+1][2] = अणु
	[PM_FPU] =   अणु 0xc0002000000000ul, 0x00001000000000ul पूर्ण,
	[PM_ISU0] =  अणु 0x00002000000000ul, 0x00000800000000ul पूर्ण,
	[PM_ISU1] =  अणु 0xc0002000000000ul, 0xc0001000000000ul पूर्ण,
	[PM_IFU] =   अणु 0xc0002000000000ul, 0x80001000000000ul पूर्ण,
	[PM_IDU] =   अणु 0x30002000000000ul, 0x00000400000000ul पूर्ण,
	[PM_GRS] =   अणु 0x30002000000000ul, 0x30000400000000ul पूर्ण,
पूर्ण;

अटल पूर्णांक घातer5_get_स्थिरraपूर्णांक(u64 event, अचिन्हित दीर्घ *maskp,
...
अणु
	...
	अगर (pmc) अणु
		...
		अगर (pmc <= 4)
			grp = (pmc - 1) >> 1;
		अन्यथा अगर (event != 0x500009 && event != 0x600005)
			वापस -1;
	पूर्ण
...
अटल स्थिर अचिन्हित पूर्णांक event_alternatives[][MAX_ALT] = अणु
	...
	अणु 0x100009, 0x200009, 0x500009 पूर्ण,	/* PM_INST_CMPL */
 ...
 * event code क्रम those that करो, or -1 otherwise.
 */
...
अटल पूर्णांक घातer5_get_alternatives(u64 event, अचिन्हित पूर्णांक flags, u64 alt[])
अणु
	पूर्णांक i, j, nalt = 1;
	s64 ae;
 ...
 */
अटल पूर्णांक घातer5_marked_instr_event(u64 event)
अणु
	पूर्णांक pmc, psel;
	...
	अगर (psel < माप(direct_event_is_marked)) अणु
		...
		अन्यथा अगर (psel == 0x1b && (pmc == 1 || pmc == 3))
			bit = 4;
	पूर्ण अन्यथा अगर ((psel & 0x58) == 0x40)
		bit = psel & 7;

	अगर (!(event & PM_BUSEVENT_MSK))
		वापस 0;
...
अटल पूर्णांक घातer5_compute_mmcr(u64 event[], पूर्णांक n_ev,
...
अणु
	...
	क्रम (i = 0; i < n_ev; ++i) अणु
		...
		पूर्ण
	पूर्ण
	अगर (pmc_grp_use[0] > 2 || pmc_grp_use[1] > 2)
		वापस -1;
	...
	क्रम (i = 0; i < n_ev; ++i) अणु
		...
		अगर (!pmc) अणु
			...
			क्रम (pmc = 0; pmc < 4; ++pmc) अणु
				...
				अगर (isbus) अणु
					अगर (grp == (byte & 1))
						अवरोध;
				पूर्ण अन्यथा अगर (pmc_grp_use[grp] < 2) अणु
					++pmc_grp_use[grp];
					अवरोध;
				पूर्ण
			पूर्ण
		...
		पूर्ण अन्यथा अगर (pmc <= 4) अणु
			/* Direct event */
			--pmc;
			अगर ((psel == 8 || psel == 0x10) && isbus && (byte & 2))
		...
		पूर्ण
		अगर (घातer5_marked_instr_event(event[i]))
...
अटल व्योम घातer5_disable_pmc(अचिन्हित पूर्णांक pmc, काष्ठा mmcr_regs *mmcr)
अणु
	अगर (pmc <= 3)
		mmcr->mmcr1 &= ~(0x7fUL << MMCR1_PMCSEL_SH(pmc));
पूर्ण

अटल पूर्णांक घातer5_generic_events[] = अणु
 ...
 * are event codes.
 */
अटल u64 घातer5_cache_events[C(MAX)][C(OP_MAX)][C(RESULT_MAX)] = अणु
	[C(L1D)] = अणु		/* 	RESULT_ACCESS	RESULT_MISS */
		[C(OP_READ)] = अणु	0x4c1090,	0x3c1088	पूर्ण,
		[C(OP_WRITE)] = अणु	0x3c1090,	0xc10c3		पूर्ण,
		[C(OP_PREFETCH)] = अणु	0xc70e7,	0		पूर्ण,
	पूर्ण,
...
अटल काष्ठा घातer_pmu घातer5_pmu = अणु
	.name			= "POWER5",
	.n_counter		= 6,
	.max_alternatives	= MAX_ALT,
	.add_fields		= 0x7000090000555ul,
	.test_adder		= 0x3000490000000ul,
	.compute_mmcr		= घातer5_compute_mmcr,
	.get_स्थिरraपूर्णांक		= घातer5_get_स्थिरraपूर्णांक,
	.get_alternatives	= घातer5_get_alternatives,
	.disable_pmc		= घातer5_disable_pmc,
	.n_generic		= ARRAY_SIZE(घातer5_generic_events),
	.generic_events		= घातer5_generic_events,
	.cache_events		= &घातer5_cache_events,
	.flags			= PPMU_HAS_SSLOT,
पूर्ण;

पूर्णांक init_घातer5_pmu(व्योम)
अणु
	अगर (!cur_cpu_spec->oprofile_cpu_type ||
	    म_भेद(cur_cpu_spec->oprofile_cpu_type, "ppc64/power5"))
		वापस -ENODEV;

	वापस रेजिस्टर_घातer_pmu(&घातer5_pmu);
पूर्ण
