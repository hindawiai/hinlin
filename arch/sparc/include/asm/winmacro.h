...
/*
 * winmacro.h: Winकरोw loading-unloading macros.
 *
 * Copyright (C) 1995 David S. Miller (davem@caip.rutgers.edu)
 */

#अगर_अघोषित _SPARC_WINMACRO_H
#घोषणा _SPARC_WINMACRO_H

#समावेश <यंत्र/ptrace.h>

/* Store the रेजिस्टर winकरोw onto the 8-byte aligned area starting
 * at %reg.  It might be %sp, it might not, we करोn't care.
 */
#घोषणा STORE_WINDOW(reg) \
	std	%l0, [%reg + RW_L0]; \
	std	%l2, [%reg + RW_L2]; \
	std	%l4, [%reg + RW_L4]; \
	std	%l6, [%reg + RW_L6]; \
...
/* Load a रेजिस्टर winकरोw from the area beginning at %reg. */
#घोषणा LOAD_WINDOW(reg) \
	ldd	[%reg + RW_L0], %l0; \
	ldd	[%reg + RW_L2], %l2; \
	ldd	[%reg + RW_L4], %l4; \
	ldd	[%reg + RW_L6], %l6; \
...
/* Loading and storing काष्ठा pt_reg trap frames. */
#घोषणा LOAD_PT_INS(base_reg) \
...
#घोषणा LOAD_PT_GLOBALS(base_reg) \
...
#घोषणा STORE_PT_GLOBALS(base_reg) \
...
#घोषणा SAVE_BOLIXED_USER_STACK(cur_reg, scratch) \
...
#घोषणा LOAD_CURRENT(dest_reg, idreg) 			\
661:	rd	%tbr, %idreg;				\
	srl	%idreg, 10, %idreg;			\
	and	%idreg, 0xc, %idreg;			\
