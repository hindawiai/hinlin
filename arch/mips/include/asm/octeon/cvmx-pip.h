...
#अगर_अघोषित __CVMX_PIP_H__
#घोषणा __CVMX_PIP_H__

#समावेश <यंत्र/octeon/cvmx-wqe.h>
#समावेश <यंत्र/octeon/cvmx-fpa.h>
#समावेश <यंत्र/octeon/cvmx-pip-defs.h>

#घोषणा CVMX_PIP_NUM_INPUT_PORTS		48
#घोषणा CVMX_PIP_NUM_WATCHERS			4

/*
 * Encodes the dअगरferent error and exception codes
 */
प्रकार क्रमागत अणु
	CVMX_PIP_L4_NO_ERR = 0ull,
	/*
	 * 1 = TCP (UDP) packet not दीर्घ enough to cover TCP (UDP)
	 * header
	 */
	...
	/*
	 * 3 = TCP/UDP length check (TCP/UDP length करोes not match IP
	 * length).
	 */
...
प्रकार क्रमागत अणु
	...
	/* 3 = malक्रमmed (packet not दीर्घ enough to cover IP hdr) */
	CVMX_PIP_IP_MAL_HDR = 3ull,
	/* 4 = malक्रमmed (packet not दीर्घ enough to cover len in IP hdr) */
	CVMX_PIP_IP_MAL_PKT = 4ull,
	/* 5 = TTL / hop count equal zero */
	CVMX_PIP_TTL_HOP = 5ull,
	/* 6 = IPv4 options / IPv6 early extension headers */
	CVMX_PIP_OPTS = 6ull
पूर्ण cvmx_pip_ip_exc_t;

/**
 * NOTES
 *	 late collision (data received beक्रमe collision)
...
प्रकार क्रमागत अणु
	...
	/*
	 * RGM+SPI+PCI 18 = malक्रमmed l2 (packet not दीर्घ enough to
	 * cover L2 hdr).
	 */
	CVMX_PIP_PIP_L2_MAL_HDR = 18L
	/*
	 * NOTES: xx = late collision (data received beक्रमe collision)
	 *	 late collisions cannot be detected by the receiver
	 *	 they would appear as JAM bits which would appear as
	 *	 bad FCS or carrier extend error which is
	 *	 CVMX_PIP_EXTEND_ERR
	 */
पूर्ण cvmx_pip_rcv_err_t;
...
पूर्ण cvmx_pip_err_t;

/**
 * Status statistics क्रम a port
 */
प्रकार काष्ठा अणु
	...
	/*
	 * Number of identअगरied L2 multicast packets.	Does not
	 * include broadcast packets.  Only includes packets whose
	 * parse mode is SKIP_TO_L2
	 */
	uपूर्णांक32_t multicast_packets;
	/*
	 * Number of identअगरied L2 broadcast packets.	Does not
 ...
 * to a packet by बाह्यal hardware.
 */
प्रकार जोड़ अणु
	uपूर्णांक64_t u64;
	काष्ठा अणु
		/*
		 * Documented as R - Set अगर the Packet is RAWFULL. If
		 * set, this header must be the full 8 bytes.
		 */
		uपूर्णांक64_t rawfull:1;
		/* Must be zero */
		uपूर्णांक64_t reserved0:5;
		/* PIP parse mode क्रम this packet */
		uपूर्णांक64_t parse_mode:2;
		/* Must be zero */
		uपूर्णांक64_t reserved1:1;
		/*
		 * Skip amount, including this header, to the
		 * beginning of the packet
		 */
		uपूर्णांक64_t skip_len:7;
		/* Must be zero */
		uपूर्णांक64_t reserved2:6;
		/* POW input queue क्रम this packet */
		uपूर्णांक64_t qos:3;
		/* POW input group क्रम this packet */
		uपूर्णांक64_t grp:4;
		/*
 ...
 */
अटल अंतरभूत व्योम cvmx_pip_config_port(uपूर्णांक64_t port_num,
					जोड़ cvmx_pip_prt_cfgx port_cfg,
					जोड़ cvmx_pip_prt_tagx port_tag_cfg)
अणु
	cvmx_ग_लिखो_csr(CVMX_PIP_PRT_CFGX(port_num), port_cfg.u64);
	cvmx_ग_लिखो_csr(CVMX_PIP_PRT_TAGX(port_num), port_tag_cfg.u64);
पूर्ण
#अगर 0
/**
 * @deprecated	    This function is a thin wrapper around the Pass1 version
 *		    of the CVMX_PIP_QOS_WATCHX CSR; Pass2 has added a field क्रम
 ...
 * QoS queue क्रम scheduling.
 *
 * @watcher:	Watcher number to configure (0 - 3).
 * @match_type: Watcher match type
 * @match_value:
 *		     Value the watcher will match against
 * @qos:	QoS queue क्रम packets matching this watcher
 */
अटल अंतरभूत व्योम cvmx_pip_config_watcher(uपूर्णांक64_t watcher,
					   cvmx_pip_qos_watch_types match_type,
					   uपूर्णांक64_t match_value, uपूर्णांक64_t qos)
अणु
	...
	cvmx_ग_लिखो_csr(CVMX_PIP_QOS_WATCHX(watcher), watcher_config.u64);
पूर्ण
#पूर्ण_अगर
/**
 ...
 * @qos:    QoS queue क्रम packets matching this watcher
 */
अटल अंतरभूत व्योम cvmx_pip_config_vlan_qos(uपूर्णांक64_t vlan_priority,
					    uपूर्णांक64_t qos)
अणु
	जोड़ cvmx_pip_qos_vlanx pip_qos_vlanx;
	pip_qos_vlanx.u64 = 0;
	pip_qos_vlanx.s.qos = qos;
	cvmx_ग_लिखो_csr(CVMX_PIP_QOS_VLANX(vlan_priority), pip_qos_vlanx.u64);
पूर्ण

/**
 * Configure the Dअगरfserv to QoS queue mapping.
 *
 * @dअगरfserv: Dअगरfserv field value (0-63)
 * @qos:      QoS queue क्रम packets matching this watcher
 */
अटल अंतरभूत व्योम cvmx_pip_config_dअगरfserv_qos(uपूर्णांक64_t dअगरfserv, uपूर्णांक64_t qos)
अणु
	जोड़ cvmx_pip_qos_dअगरfx pip_qos_dअगरfx;
	pip_qos_dअगरfx.u64 = 0;
	pip_qos_dअगरfx.s.qos = qos;
	cvmx_ग_लिखो_csr(CVMX_PIP_QOS_DIFFX(dअगरfserv), pip_qos_dअगरfx.u64);
पूर्ण
...
अणु
	जोड़ cvmx_pip_stat_ctl pip_stat_ctl;
	जोड़ cvmx_pip_stat0_prtx stat0;
	जोड़ cvmx_pip_stat1_prtx stat1;
	जोड़ cvmx_pip_stat2_prtx stat2;
	जोड़ cvmx_pip_stat3_prtx stat3;
	जोड़ cvmx_pip_stat4_prtx stat4;
	जोड़ cvmx_pip_stat5_prtx stat5;
	जोड़ cvmx_pip_stat6_prtx stat6;
	जोड़ cvmx_pip_stat7_prtx stat7;
	जोड़ cvmx_pip_stat8_prtx stat8;
	जोड़ cvmx_pip_stat9_prtx stat9;
	जोड़ cvmx_pip_stat_inb_pktsx pip_stat_inb_pktsx;
	जोड़ cvmx_pip_stat_inb_octsx pip_stat_inb_octsx;
	जोड़ cvmx_pip_stat_inb_errsx pip_stat_inb_errsx;

	pip_stat_ctl.u64 = 0;
	pip_stat_ctl.s.rdclr = clear;
	cvmx_ग_लिखो_csr(CVMX_PIP_STAT_CTL, pip_stat_ctl.u64);

	stat0.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT0_PRTX(port_num));
	stat1.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT1_PRTX(port_num));
	stat2.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT2_PRTX(port_num));
	stat3.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT3_PRTX(port_num));
	stat4.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT4_PRTX(port_num));
	stat5.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT5_PRTX(port_num));
	stat6.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT6_PRTX(port_num));
	stat7.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT7_PRTX(port_num));
	stat8.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT8_PRTX(port_num));
	stat9.u64 = cvmx_पढ़ो_csr(CVMX_PIP_STAT9_PRTX(port_num));
	pip_stat_inb_pktsx.u64 =
	    cvmx_पढ़ो_csr(CVMX_PIP_STAT_INB_PKTSX(port_num));
	pip_stat_inb_octsx.u64 =
	    cvmx_पढ़ो_csr(CVMX_PIP_STAT_INB_OCTSX(port_num));
	pip_stat_inb_errsx.u64 =
	    cvmx_पढ़ो_csr(CVMX_PIP_STAT_INB_ERRSX(port_num));
	...
	अगर (cvmx_octeon_is_pass1()) अणु
		 ...
		 * करोn't work.
		 */
		अगर (status->inb_packets > status->packets)
			status->dropped_packets =
			    status->inb_packets - status->packets;
		अन्यथा
			status->dropped_packets = 0;
		अगर (status->inb_octets - status->inb_packets * 4 >
		    status->octets)
 ...
 *
 * @पूर्णांकerface: Interface to configure (0 or 1)
 * @invert_result:
 ...
 */
अटल अंतरभूत व्योम cvmx_pip_config_crc(uपूर्णांक64_t पूर्णांकerface,
				       uपूर्णांक64_t invert_result, uपूर्णांक64_t reflect,
				       uपूर्णांक32_t initialization_vector)
अणु
	अगर (OCTEON_IS_MODEL(OCTEON_CN38XX) || OCTEON_IS_MODEL(OCTEON_CN58XX)) अणु
		जोड़ cvmx_pip_crc_ctlx config;
		जोड़ cvmx_pip_crc_ivx pip_crc_ivx;

		config.u64 = 0;
		config.s.invres = invert_result;
		config.s.reflect = reflect;
		cvmx_ग_लिखो_csr(CVMX_PIP_CRC_CTLX(पूर्णांकerface), config.u64);

		pip_crc_ivx.u64 = 0;
		pip_crc_ivx.s.iv = initialization_vector;
		cvmx_ग_लिखो_csr(CVMX_PIP_CRC_IVX(पूर्णांकerface), pip_crc_ivx.u64);
	पूर्ण
पूर्ण

/**
 * Clear all bits in a tag mask. This should be called on
 * startup beक्रमe any calls to cvmx_pip_tag_mask_set. Each bit
 * set in the final mask represent a byte used in the packet क्रम
 * tag generation.
 *
 * @mask_index: Which tag mask to clear (0..3)
 */
अटल अंतरभूत व्योम cvmx_pip_tag_mask_clear(uपूर्णांक64_t mask_index)
अणु
	uपूर्णांक64_t index;
	जोड़ cvmx_pip_tag_incx pip_tag_incx;
	pip_tag_incx.u64 = 0;
	pip_tag_incx.s.en = 0;
	क्रम (index = mask_index * 16; index < (mask_index + 1) * 16; index++)
		cvmx_ग_लिखो_csr(CVMX_PIP_TAG_INCX(index), pip_tag_incx.u64);
पूर्ण
 ...
 *
 * @mask_index: Which tag mask to modअगरy (0..3)
 * @offset: Offset पूर्णांकo the biपंचांगask to set bits at. Use the GCC macro
 *	    दुरत्व() to determine the offsets पूर्णांकo packet headers.
 *	    For example, दुरत्व(ethhdr, protocol) वापसs the offset
 *	    of the ethernet protocol field.  The biपंचांगask selects which
 *	    bytes to include the tag, with bit offset X selecting
 *	    byte at offset X from the beginning of the packet data.
 * @len:    Number of bytes to include. Usually this is the माप()
 *	    the field.
 */
अटल अंतरभूत व्योम cvmx_pip_tag_mask_set(uपूर्णांक64_t mask_index, uपूर्णांक64_t offset,
					 uपूर्णांक64_t len)
अणु
	जबतक (len--) अणु
		जोड़ cvmx_pip_tag_incx pip_tag_incx;
		uपूर्णांक64_t index = mask_index * 16 + offset / 8;
		pip_tag_incx.u64 = cvmx_पढ़ो_csr(CVMX_PIP_TAG_INCX(index));
