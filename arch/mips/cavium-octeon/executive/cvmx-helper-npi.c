 ...
 * connected to it. The NPI पूर्णांकerface should still be करोwn
 * after this call.
 *
 ...
 */
पूर्णांक __cvmx_helper_npi_probe(पूर्णांक पूर्णांकerface)
अणु
#अगर CVMX_PKO_QUEUES_PER_PORT_PCI > 0
	अगर (OCTEON_IS_MODEL(OCTEON_CN38XX) || OCTEON_IS_MODEL(OCTEON_CN58XX))
		वापस 4;
	अन्यथा अगर (OCTEON_IS_MODEL(OCTEON_CN56XX)
		 && !OCTEON_IS_MODEL(OCTEON_CN56XX_PASS1_X))
		/* The packet engines didn't exist beक्रमe pass 2 */
		वापस 4;
 ...
 */
पूर्णांक __cvmx_helper_npi_enable(पूर्णांक पूर्णांकerface)
अणु
	/*
	 * On CN50XX, CN52XX, and CN56XX we need to disable length
	 * checking so packet < 64 bytes and jumbo frames करोn't get
	 * errors.
	 */
	अगर (!OCTEON_IS_MODEL(OCTEON_CN3XXX) &&
		...
		क्रम (port = 0; port < num_ports; port++) अणु
			...
			पूर्णांक ipd_port =
			    cvmx_helper_get_ipd_port(पूर्णांकerface, port);
			port_cfg.u64 =
			    cvmx_पढ़ो_csr(CVMX_PIP_PRT_CFGX(ipd_port));
			port_cfg.s.maxerr_en = 0;
			port_cfg.s.minerr_en = 0;
			cvmx_ग_लिखो_csr(CVMX_PIP_PRT_CFGX(ipd_port),
				       port_cfg.u64);
		पूर्ण
	पूर्ण

	/* Enables are controlled by the remote host, so nothing to करो here */
	वापस 0;
पूर्ण
