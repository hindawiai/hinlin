...
अणु
	...
	अगर (xs == ys) अणु
		...
		अगर (xm >> (DP_FBITS + 1 + 3)) अणु /* carry out */
			xm = XDPSRS1(xm);	/* shअगरt preserving sticky */
			xe++;
		पूर्ण
	पूर्ण अन्यथा अणु
		अगर (xm >= ym) अणु
		...
		अगर (xm == 0) अणु
			अगर (ieee754_csr.rm == FPU_CSR_RD)
				वापस ieee754dp_zero(1);	/* round negative inf. => sign = -1 */
			अन्यथा
				वापस ieee754dp_zero(0);	/* other round modes   => sign = 1 */
		पूर्ण
