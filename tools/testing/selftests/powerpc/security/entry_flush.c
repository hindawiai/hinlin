...
पूर्णांक entry_flush_test(व्योम)
अणु
	अक्षर *p;
	...
	पूर्णांक entry_flush, entry_flush_orig;
...
again:
	...
	अगर (entry_flush && v.l1d_misses >= l1d_misses_expected)
		passes++;
	अन्यथा अगर (!entry_flush && v.l1d_misses < (l1d_misses_expected / 2))
		passes++;
	...
	अगर (passes < repetitions) अणु
		म_लिखो("FAIL (L1D misses with entry_flush=%d: %llu %c %lu) [%d/%d failures]\n",
		...
		rc = 1;
	पूर्ण अन्यथा अणु
		म_लिखो("PASS (L1D misses with entry_flush=%d: %llu %c %lu) [%d/%d pass]\n",
		       entry_flush, l1d_misses_total, entry_flush ? '>' : '<',
		       entry_flush ? repetitions * l1d_misses_expected :
		       repetitions * l1d_misses_expected / 2,
		       passes, repetitions);
	...
	अगर (entry_flush == entry_flush_orig) अणु
		entry_flush = !entry_flush_orig;
		अगर (ग_लिखो_debugfs_file("powerpc/entry_flush", entry_flush) < 0) अणु
...
अणु
	वापस test_harness(entry_flush_test, "entry_flush_test");
पूर्ण
