...
/*
 * Ptrace test क्रम VMX/VSX रेजिस्टरs in the TM Suspend context
 *
...
__attribute__((used)) व्योम load_vsx_new(व्योम)
अणु
...
पूर्णांक trace_पंचांग_spd_vsx(pid_t child)
अणु
...
पूर्णांक ptrace_पंचांग_spd_vsx(व्योम)
अणु
	pid_t pid;
	...
	क्रम (i = 0; i < 128; i++) अणु
		fp_load[i] = 1 + अक्रम();
		fp_load_new[i] = 1 + 2 * अक्रम();
		fp_load_ckpt[i] = 1 + 3 * अक्रम();
		fp_load_ckpt_new[i] = 1 + 4 * अक्रम();
	पूर्ण
...
अणु
	वापस test_harness(ptrace_पंचांग_spd_vsx, "ptrace_tm_spd_vsx");
पूर्ण
