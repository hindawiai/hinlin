<शैली गुरु>
// SPDX-License-Identअगरier: GPL-2.0
#समावेश <test_progs.h>

#घोषणा IFINDEX_LO 1
#घोषणा XDP_FLAGS_REPLACE		(1U << 4)

व्योम test_xdp_attach(व्योम)
अणु
	__u32 duration = 0, id1, id2, id0 = 0, len;
	काष्ठा bpf_object *obj1, *obj2, *obj3;
	स्थिर अक्षर *file = "./test_xdp.o";
	काष्ठा bpf_prog_info info = अणुपूर्ण;
	पूर्णांक err, fd1, fd2, fd3;
	DECLARE_LIBBPF_OPTS(bpf_xdp_set_link_opts, opts,
			    .old_fd = -1);

	len = माप(info);

	err = bpf_prog_load(file, BPF_PROG_TYPE_XDP, &obj1, &fd1);
	अगर (CHECK_FAIL(err))
		वापस;
	err = bpf_obj_get_info_by_fd(fd1, &info, &len);
	अगर (CHECK_FAIL(err))
		जाओ out_1;
	id1 = info.id;

	err = bpf_prog_load(file, BPF_PROG_TYPE_XDP, &obj2, &fd2);
	अगर (CHECK_FAIL(err))
		जाओ out_1;

	स_रखो(&info, 0, माप(info));
	err = bpf_obj_get_info_by_fd(fd2, &info, &len);
	अगर (CHECK_FAIL(err))
		जाओ out_2;
	id2 = info.id;

	err = bpf_prog_load(file, BPF_PROG_TYPE_XDP, &obj3, &fd3);
	अगर (CHECK_FAIL(err))
		जाओ out_2;

	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, fd1, XDP_FLAGS_REPLACE,
				       &opts);
	अगर (CHECK(err, "load_ok", "initial load failed"))
		जाओ out_बंद;

	err = bpf_get_link_xdp_id(IFINDEX_LO, &id0, 0);
	अगर (CHECK(err || id0 != id1, "id1_check",
		  "loaded prog id %u != id1 %u, err %d", id0, id1, err))
		जाओ out_बंद;

	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, fd2, XDP_FLAGS_REPLACE,
				       &opts);
	अगर (CHECK(!err, "load_fail", "load with expected id didn't fail"))
		जाओ out;

	opts.old_fd = fd1;
	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, fd2, 0, &opts);
	अगर (CHECK(err, "replace_ok", "replace valid old_fd failed"))
		जाओ out;
	err = bpf_get_link_xdp_id(IFINDEX_LO, &id0, 0);
	अगर (CHECK(err || id0 != id2, "id2_check",
		  "loaded prog id %u != id2 %u, err %d", id0, id2, err))
		जाओ out_बंद;

	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, fd3, 0, &opts);
	अगर (CHECK(!err, "replace_fail", "replace invalid old_fd didn't fail"))
		जाओ out;

	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, -1, 0, &opts);
	अगर (CHECK(!err, "remove_fail", "remove invalid old_fd didn't fail"))
		जाओ out;

	opts.old_fd = fd2;
	err = bpf_set_link_xdp_fd_opts(IFINDEX_LO, -1, 0, &opts);
	अगर (CHECK(err, "remove_ok", "remove valid old_fd failed"))
		जाओ out;

	err = bpf_get_link_xdp_id(IFINDEX_LO, &id0, 0);
	अगर (CHECK(err || id0 != 0, "unload_check",
		  "loaded prog id %u != 0, err %d", id0, err))
		जाओ out_बंद;
out:
	bpf_set_link_xdp_fd(IFINDEX_LO, -1, 0);
out_बंद:
	bpf_object__बंद(obj3);
out_2:
	bpf_object__बंद(obj2);
out_1:
	bpf_object__बंद(obj1);
पूर्ण
